...
do
    ...
    then
        ...
        echo "submitting $FF"
        echo "cat ${seqdir}_all_files.txt | /usr/bin/time -v ${metagraph} build -v --parallel ${threads} --canonical -k ${K} --mem-cap-gb $((${mem} / 2000)) -o ${outdir}/graph_merged_k${K} --suffix $F 2>&1 | tee ${outdir}/build_k${K}_$F.log" | bsub -J tc_g_${FF} -oo ${outdir}/build_k${K}_${FF}.lsf.log -We 12:00 -n $threads -M $mem -R "rusage[mem=${pmem}]" -R "span[hosts=1]"
    fi
done

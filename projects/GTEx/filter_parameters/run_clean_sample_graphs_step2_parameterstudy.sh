...
do
    ...
    echo "/usr/bin/time -v $metagraph build -k $K -o ${outgraph%.dbg} --canonical -v ${inseq} && $metagraph assemble --unitigs -o ${outseq%.fasta.gz} $outgraph" | bsub -J ms_clean -o ${logfname} -We 8:00 -n 1 -M $mem -R "rusage[mem=${pmem}]" -R "span[hosts=1]"
done

...
outdir=${basedir}/gtex/output_k${K}_clean
mkdir -p $outdir
...
echo "kmc_file=\"\$(sed -n \${LSB_JOBINDEX}p ${filelist})\"; \
      ...
      ${metagraph} build -p $threads -k $K -o \${outbase} --canonical --count-kmers \${kmc_file}; \
      ${metagraph} clean --prune-tips $(($K * 2)) --prune-unitigs 0 --fallback 2 -o \${outbase}.clean \${outbase}.dbg; \

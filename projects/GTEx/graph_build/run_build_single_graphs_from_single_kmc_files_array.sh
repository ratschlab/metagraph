...
echo "kmc_file=\"\$(sed -n \${LSB_JOBINDEX}p ${filelist})\"; \
      ...
      ../../../metagraph/build/metagraph build -p $threads -k $K -o ${outdir}/\${x%.kmc_suf} --kmc \${kmc_file}" | bsub -J contigs_k${K}[1-$(wc -l $filelist | cut -f1 -d ' ')]%400 -o graph_build_gtex_k${K}.lsf -We 20:00 -n $threads -R "rusage[mem=${mem}]" -R "span[hosts=1]"

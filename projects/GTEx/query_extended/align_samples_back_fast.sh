...
do
    ...
    echo "(/usr/bin/time -v $metagraph query --align-min-seed-length 19 --fast --align --discovery-fraction 0.0 --count-labels -v -i $graph -a $anno -p $threads $fq1 $fq2 | gzip > $outfile) && touch $donefile" | bsub -M $mem -n $threads -R "rusage[mem=${pmem}]" -We 48:00 -n 1 -J map_gtex -oo $logfile  #-R "select[model==XeonGold_6150]"

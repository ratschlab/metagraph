...
for l,line in enumerate(gzip.open(infname, 'r')):
    ...
    if os.path.basename(infname).startswith('realign'):
        ...
        READLEN = len(sl[1].split(':')[1])
    else:
        sample_counts = sl[2:]

    if sample_counts[0].split(':')[0] != sample:
        matches = int(sample_counts[0].split(':')[1])
    elif len(sample_counts) > 1:
        matches = int(sample_counts[1].split(':')[1])
    else:
        unaligned += 1
        continue

    e = READLEN - 40 - matches
    if e < 0:
        import pdb
        pdb.set_trace()
    try:

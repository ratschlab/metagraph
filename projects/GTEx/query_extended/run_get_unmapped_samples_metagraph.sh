...
do
    ...
    outfname=${fname%.txt.gz}.unmapped.txt

    if [ ! -f ${outfname} ]
    then
        echo "python $(pwd)/get_unmapped_samples_metagraph.py $fname > $outfname" | bsub -M $mem -n $threads -R "rusage[mem=${pmem}]" -We 20:00 -n 1 -J sum_gtex -o /dev/null
    else

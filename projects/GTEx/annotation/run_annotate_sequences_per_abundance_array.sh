...
if [ ! -f \${outbase}.column.annodbg ]; then $(pwd)/../../../metagraph/build/metagraph annotate -i $graph -o \${outbase} --anno-label \${fbase} \${fname}; fi" | bsub -M $mem -n 1 -R "rusage[mem=${mem}]" -We 24:00 -J annoGT${K}[${from}-${to}]%400 -o $logfile

...
class DBG_succ {
    ...
    private:
        ...
        struct BranchInfo {
            ...
            BranchInfo(uint64_t nodeId_ = 0, TAlphabet lastEdge_ = 0):
                nodeId(nodeId_),
                lastEdge(lastEdge_) {}
        };
        ...
        size_t traverseGraph(std::vector<JoinInfo> &joins, std::map<std::pair<uint64_t, TAlphabet>, uint64_t> &branchMap, std::ofstream &SQLstream) {
            ...
            while (out > 0 || branchnodes.size() > 0) {
                ...
                //fprintf(stderr, "nodeId %lu out %lu\n", nodeId, out);
                        ...
                        if (next > 0) {
                            ...
                            if (!visited.at(next)) {
                                ...
                                if (cnt < out && next != nodeId) {
                                    ...
                                    branchnodes.push(BranchInfo(nodeId, lastEdge));
                                    if (debug)
            ...
            // for completeness
            if (seqan::length(sequence) > 0)
                finish_sequence(sequence, seqId, SQLstream);
            else
                seqCnt--;
        ...
        void allelesFromSeq(seqan::String<Dna5F>seq, unsigned int f, std::vector<JoinInfo> &joins, std::map<std::pair<uint64_t, TAlphabet>, uint64_t> &branchMap, std::ofstream &SQLstream, bool isRefRun = false, size_t seqNum = 0) {
            ...
            while (true) {
                ...
                fprintf(stderr, "nodeVal %lu seqVal %lu nodeId %lu seqId %lu\n", nodeVal, seqVal, nodeId, seqId);
                assert(nodeVal % alph_size == seqVal);
                if (seqPos + 1 == length(seq))
                    break;
                seqValNext = ordValue(seq[seqPos + 1]) + 1;
                seqPos++;

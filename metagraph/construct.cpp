...
namespace construct {
    ...
    void add_seq_alt(DBG_succ* G, kstring_t &seq, bool bridge, unsigned int parallel, std::string suffix) {
        ...
        */
        ...
        //clock_t start = clock();
        //std::cerr << "Loading G->kmers\n";
        if (!G->kmers.size()) {
            seqtokmer(G->kmers, "$", 1, G->k, nt_lookup);
            G->kmers.push_back(stokmer(std::string(G->k-1,'X') + std::string("$$"), nt_lookup));
        }
        seqtokmer(G->kmers, seq.s, seq.l, G->k, nt_lookup, bridge, parallel, suffix);
    ...
    void construct_succ(DBG_succ* G, unsigned int parallel) {
        ...
        delete G->last;
        delete G->W;
        G->last = new BitBTree(G->kmers.size(), true);
        //sdsl::int_vector<> wbv(kmers.size(), 0, strlen(alphabet));
        
        std::vector<uint8_t> Wvec(G->kmers.size());
        size_t lastlet=0;
        std::cerr << "\n";
        ...
        #pragma omp parallel num_threads(parallel)
        {
            ...
            for (size_t i=0;i<G->kmers.size();++i) {
                ...
                if (!curW && i)
                    G->p=i;
                if (i) {
                    //uint64_t cF=get_alphabet_number(getPos(G->kmers[i], k-1, alphabet, alph_size));
                    //#pragma omp critical
                    ...
                    /*
                    size_t j;
                    for (j=i-1;getW(G->kmers[j])!= curW;--j) {
                        if (!j)
                            break;
                    }
                    if (getW(G->kmers[j]) == curW && compare_kmer_suffix(G->kmers[j], G->kmers[i], 1)) {
                        curW += alph_size;
                    }
                    */
                }
        ...
        G->W = new WaveletTree(Wvec, 4, parallel);
        assert(G->W->size() == Wvec.size());
        assert((*(G->W))[3] == Wvec[3]);
        Wvec.clear();
        ...
        for (size_t i=0;i<G->kmers.size();++i) {
            ...
            /*
            char *curseq = kmertos(G->kmers[i], G->alphabet, G->alph_size);
            std::cerr << "-" << (*(G->last))[i] << " " << curseq << " " << G->get_alphabet_symbol(Wvec[i]) << (Wvec[i]>=G->alph_size ? "-" : "") << "\n"; 
            free(curseq);
            */
            if (cF != G->alphabet[lastlet]) {
                //std::cerr << cF << " " << lastlet << " " << i-1 << "\n";
                for (lastlet++; lastlet < G->alph_size; lastlet++) {
                    G->F[lastlet]=i-1;
                    if (G->alphabet[lastlet]==cF) {
                        break;
                    }
                }
            }
        ...
        fclose(sfile);
        fprintf(stdout, "edges %lu / nodes %lu/ %s\n", G->get_edge_count(), G->get_node_count(), line);
    }

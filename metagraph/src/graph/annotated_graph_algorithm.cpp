...
mask_nodes_by_label_dual(std::shared_ptr<const DeBruijnGraph> graph_ptr,
    ...
    if (config.test_type == "poisson_bayes" || config.test_type == "nbinom_exact") {
        ...
        auto get_rp = [&](const auto &generate) {
            ...
            long double r_guess = mu * mu / (var - mu);
        ...
        generate_clean_rows([&](uint64_t row_i, const auto &row, uint64_t bucket_idx) {
            ...
            if (config.test_type == "poisson_binom") {
                ...
                if (config.test_type == "poisson_exact") {
                    mu1 = static_cast<long double>(in_kmers) / nelem;
                    mu2 = static_cast<long double>(out_kmers) / nelem;
                }
                ...
                for (const auto &[j, c] : row) {
                    ...
                    if (config.test_type == "poisson_bayes") {
                        found[j] = true;
                        const auto &[r, p] = nb_params[j];
                        long double lambda_j = (1.0 - p) * (r + c - 1);
                        if (groups[j] == Group::OUT || groups[j] == Group::BOTH)
                            mu2 += lambda_j;
                        ...
                        if (config.test_type == "poisson_bayes") {
                            for (size_t j = 0; j < found.size(); ++j) {
                                if (!found[j]) {
                                    const auto &[r, p] = nb_params[j];
                                    if (r > 1.0) {
                                        long double lambda_j = (1.0 - p) * (r - 1);
                                        if (groups[j] == Group::OUT || groups[j] == Group::BOTH)
                                            mu2 += lambda_j;
    ...
    if (m.size() > 1) {
        ...
        while (jt != m.end()) {
            ...
            if (jt->first - it->first > 1e-5) {
                common::logger->error("min p-vals not sorted: {} !>= {}", it->first, jt->first);
    ...
    generate_clean_rows([&](uint64_t row_i, const auto &row, uint64_t bucket_idx) {
        ...
        if (config.test_type == "poisson_binom") {
            ...
            if (n > 0) {
                ...
                auto get_deviance = [&](size_t s, size_t t) {
                    if (s == 0)
                        return (r_a*logl(r_a) + (n+r_b)*logl(n+r_b)) * -2.0;

                    if (t == 0)
                        return (r_b*logl(r_b) + (n+r_a)*logl(n+r_a)) * -2.0;

                    return (logl(s)*s - (r_a+s)*logl(r_a+s) + logl(t)*t - (r_b+t)*logl(r_b+t)) * 2.0;
                };

...
std::tuple<long double, long double, long double, long double> get_rp(const Generator &generate) {
    long double mu = 0;
    long double var = 0;
    size_t total = 0;
    ...
    auto get_dl_a = [&](long double a) {
        long double dl = mu * mu * total / (1.0 + a * mu) + total * logl(1 + a * mu) / a / a + total / a / a * boost::math::digamma(1.0 / a);
        generate([&](auto k, auto c) {
    ...
    try {
        auto [r_min, r_max] = boost::math::tools::bisect(
            get_dl_r, std::numeric_limits<double>::min(), 1.0, boost::math::tools::eps_tolerance<long double>(5)
        );
        r1 = (r_min + r_max) / 2.0;
    ...
    try {
        auto [a_min, a_max] = boost::math::tools::bisect(
            get_dl_a, std::numeric_limits<double>::min(), 1.0, boost::math::tools::eps_tolerance<long double>(5)
        );
...
mask_nodes_by_label_dual(std::shared_ptr<const DeBruijnGraph> graph_ptr,
    ...
    auto combine_pvals = [dist=boost::math::cauchy()](const std::vector<long double> &pvals) {
    ...
    if (config.test_by_unitig && config.test_type != "notest") {
        ...
        common::logger->trace("In: r: {}\tp: {}\t mu: {}\tsample var: {}\tvar: {}", r_a, p_a, mu_a, var_a, mu_a / p_a);
        ...
        common::logger->trace("Allocating p-value storage");
        std::vector<std::tuple<long double, Group, size_t>> pvals(counts.size(), std::make_tuple(1.0, Group::OTHER, counts.size()));

        common::logger->trace("Calculating p-values");
        size_t rows_per_update = 10000;
        ProgressBar progress_bar(pvals.size(), "Streaming unitigs", std::cerr, !common::get_verbose());
        ...
        for (size_t i = 0; i < counts.size(); ++i) {
            ...
            auto get_deviance_a = [&](long double s) {
                ...
                return ((logl(s) - log1pl(-p_a)) * s - (r_a + s) * logl(r_a + s) + r_a * (logl(r_a) - logl(p_a))) * 2.0;
            };
            ...
            auto get_deviance_b = [&](long double t) {
                ...
                return ((logl(t) - log1pl(-p_b)) * t - (r_b + t) * logl(r_b + t) + r_b * (logl(r_b) - logl(p_b))) * 2.0;
            };
            ...
            long double min_dev = get_deviance(midpoint, static_cast<long double>(n) - midpoint);
            ...
            if (in_dev > min_dev) {
                ...
                pval = 0.0;
                idx = i;
                long double base = nb_base + (lgammal(n + 1) - lgammal(r_n + n) - n*log1pl(-p_n)) / logl(2.0);
...

                {
                    size_t s = 0;
                    size_t t = n;
                    long double sbase = (lgammal(r_a+s)+lgammal(r_b+t)-lgammal(s+1)-lgammal(t+1)) / logl(2.0) + s*l1pa + t*l1pb;
                    pval += exp2(base + sbase);
                    for (++s; s <= n; ++s) {
                        long double dev = get_deviance(s, t - 1);
                        if (dev >= in_dev) {
                            sbase += log2l(r_a + s) - log2l(r_b + t) - log2l(s + 1) + log2l(t + 1) + l1pa - l1pb;
                            --t;
                            pval += exp2(base + sbase);
        ...
        if (config.test_type == "nbinom_exact") {
            ...
            for (size_t j = 0; j < groups.size(); ++j) {
                ...
                if (hist.size()) {
                    ...
                    common::logger->trace("{}: size: {}\tmax_val: {}\tsample mean: {}\tsample var: {}\tmu: {}\tvar: {}\tmle: r: {}\tp: {}",
                                        j, sums[j], (hist.end() - 1)->first,
                                        mu, var,
                                        r * (1-p)/p, r*(1-p)/p/p,
                                        r, p);
                }
            ...
            if (config.test_type == "nbinom_exact") {
                ...
                // long double var_null = var_a + var_b; // the two are independent, so their covariance is 0
                ...
                nb_base = (lgammal(nb_params_null.first) - lgammal(nb_params_a.first) - lgammal(nb_params_b.first)) / logl(2.0);
                nb_base += nb_params_a.first*log2l(nb_params_a.second) + nb_params_b.first*log2l(nb_params_b.second)-nb_params_null.first*log2l(nb_params_null.second);
            }
        }

        using RowGenerator = std::function<void(const std::function<void(size_t, uint64_t)>&)>;
        using GetLambda = std::function<std::pair<long double, long double>(size_t, const RowGenerator&)>;
        ...
        if (config.test_type == "poisson_bayes") {
            ...
            common::logger->trace("Lognormal MoM\tin:\tmu: {}\tvar: {}", pln_params_a.first, pln_params_a.second);
            common::logger->trace("Lognormal MoM\tout:\tmu: {}\tvar: {}", pln_params_b.first, pln_params_b.second);
            common::logger->trace("Lognormal MoM\tnull:\tmu: {}\tvar: {}", pln_params_null.first, pln_params_null.second);
        ...
        std::vector<std::pair<tsl::hopscotch_map<size_t, long double>,
                            tsl::hopscotch_map<size_t, long double>>> lambdas(num_threads + 1);
        if (config.test_type == "poisson_bayes") {
            get_lambda = [&](size_t bucket_idx, const RowGenerator &generate) -> std::pair<long double, long double> {
                size_t in_sum = 0;
                size_t out_sum = 0;
        ...
        if (config.test_type == "poisson_binom") {
            ...
            for (size_t n = 1; n < pb_pvals.size(); ++n) {
                ...
                long double local_min_pval = std::min(pb_pvals[n][front],
                                                    pb_pvals[n].back());
        ...
        if (config.test_type == "bb_hypergeometric") {
            auto get_ab = [&](long double n, long double n2) {
                ...
                long double a = mu * (n * mu - mu * mu - var) / n / (var - mu + mu * mu / n);
                long double b = a * (n / mu - 1);
        ...
        if (config.test_type == "gpoisson") {
            ...
            long double var_a = static_cast<long double>(in_sq_kmers) / nelem - lambda_a * lambda_a;
            long double var_b = static_cast<long double>(out_sq_kmers) / nelem - lambda_b * lambda_b;
            ...
            auto get_omega = [&](long double lambda, long double var, const auto &generate) {
                ...
                if (1.0 <= val) {
                    common::logger->trace("Invalid omega: {}\tlambda: {}\tvar: {}", 1.0 - val, lambda, var);
            ...
            common::logger->trace("Generalized Poisson In:\tlambda: {}\tomega: {}\tsample variance: {}\tvar: {}",
                                  lambda_a, omega_a, var_a, lambda_a / pow(1.0 - omega_a, 2.0));
            common::logger->trace("Generalized Poisson Out:\tlambda: {}\tomega: {}\tsample variance: {}\tvar: {}",
            ...
            long double gp_base = (log1pl(-omega_a) + log1pl(-omega_b) - (1.0 - omega_a) * lambda_a - (1.0 - omega_b) * lambda_b) / logl(2.0)
            ...
            ProgressBar progress_bar(row_sums_n.size(), "Precomputing p-values", std::cerr, !common::get_verbose());
        ...
        if (config.test_type == "pig") {
            ...
            auto get_lp = [&](long double x, long double mu, long double lambda) {
                ...
                if (x == 0.0) {
                    if (p > 0) {
                        common::logger->error("mu: {}\tlambda: {}\tx: {}\tlogp: {}", mu, lambda, x, p);
                ...
                if (p > 1.0) {
                    common::logger->error("mu: {}\tlambda: {}\tx: {}\tlogp: {}", mu, lambda, x, logl(p));
                    throw std::runtime_error("Fail");
                }
            ...
            for (size_t i = 0; i < groups.size(); ++i) {
                ...
                long double val = 0.0;
                try {
                    std::tie(lambda, val) = boost::math::tools::brent_find_minima([&](long double lambda) {
                        long double val = 0.0;
                ...
                try {
                    auto [phi, val2] = boost::math::tools::brent_find_minima([&](long double phi) {
                        long double val = 0.0;
                        for (const auto &[k, c] : hists[i]) {
                ...
                common::logger->trace("i: {}\tmu: {}\tsample var: {}\tlambda: {}\tvar: {}",
                                        i, mu, var, lambda, mu + mu * mu * mu / lambda);
                lambdas[i] = lambda;
            }
            ...
            // long double var_n = static_cast<long double>(total_sq_kmers) / nelem - mu_n * mu_n;
            // long double lambda = mu_n * mu_n * mu_n / (var_n - mu_n);
        ...
        if (config.test_type != "notest" && config.test_type != "nbinom_exact" && config.test_type != "pig") {
            ...
            generate_clean_rows([&](uint64_t row_i, const auto &row, uint64_t bucket_idx) {
                ...
                if (config.test_type == "poisson_binom") {
                    ...
                    if (n > 0) {
                        ...
                        if (ms[bucket_idx][n].second == 0) {
                            ...
                            long double base = nb_base + (lgammal(n + 1) - lgammal(r_n + n)) / log2l(2.0) - n*l21p;
                            auto get_pval = [&](int64_t s) {
                                int64_t t = n - s;
                                long double sbase = (lgammal(r_a+s)+lgammal(r_b+t)-lgammal(s+1)-lgammal(t+1) + s * log1pl(-p_a) + t*log1pl(-p_b)) / logl(2.0);
                ...
                } else if (config.test_type == "hypergeometric" || config.test_type == "poisson_bayes") {
                    size_t n = 0;
                    for (const auto &[j, c] : row) {
                        n += c;
                    }
                    if (n > 0) {
                        ...
                        // if (ms[bucket_idx][n].second == 0) {
                            ...
                            boost::math::binomial_distribution dist(static_cast<long double>(n), mu1 / (mu1 + mu2));
                    ...
                    if (n > 0) {
                        ...
                        if (ms[bucket_idx][n].second == 0) {
                            ...
                            auto get_lp = [&](long double x, long double mu, long double lambda, long double base) {
                                long double phi = 1.0L / lambda;
                                int64_t i;
                                long double p, pi1m, pi2m;
                                long double twophi = phi + phi;
                                ...
                                if (x == 0.0) {
                                    if (p > 0) {
                                        common::logger->error("mu: {}\tlambda: {}\tn: {}\tx: {}\tlogp: {}", mu, lambda, n, x, p);
                                        throw std::runtime_error("Fail");
                                    }
                                    return p;
                                }
                                ...
                                if (p > 1.0) {
                                    common::logger->error("mu: {}\tlambda: {}\tn: {}\tx: {}\tlogp: {}", mu, lambda, n, x, logl(p));
                                    throw std::runtime_error("Fail");
                                }
                                ...
                                //             - 0.5*(x - 0.5) * (logl(2.0) - logl(lambda) + logl(1.0 + lambda / 2 / mu / mu))
                                //             + logl(cyl_bessel_kl(x - 0.5, sqrtl(lambda * 2 * (1.0 + lambda / 2 / mu / mu))));
                                // }
                                // if (lprob > 0) {
                                //     common::logger->error("mu: {}\tlambda: {}\tx: {}\tn: {}\tlog(pval): {}\tpval: {}",
                            ...
                            auto get_lprob = [&](int64_t s) {
                                return get_lp(s, mu_a, lambda_a, pig_base_a) + get_lp(n - s, mu_b, lambda_b, pig_base_b) - base_lprob;
                            };
                    ...
                    if (n > 0) {
                        ...
                        if (ms[bucket_idx][n].second == 0) {
                            ...
                            long double nx = static_cast<long double>(in_kmers);
                            long double ny = static_cast<long double>(out_kmers);
                            long double base = (lgammal(n + 1) + lgammal(nx + ny - n + 1)
                                                - lgammal(a_n + n) - lgammal(b_n + nx + ny - n)) / logl(2.0) + bb_base;
                            int64_t s = 0;
                            long double pval0 = (-lgammal(s + 1) - lgammal(nx - s + 1)
                                                - lgammal(n - s + 1) - lgammal(ny - n + s + 1)
                                                + lgammal(a_a + s) + lgammal(b_a + nx - s) + lgammal(a_b + n - s) + lgammal(b_b + ny - n + s)) / logl(2.0) + base;
                            s = n;
                            long double pvaln = (-lgammal(s + 1) - lgammal(nx - s + 1)
                                                - lgammal(n - s + 1) - lgammal(ny - n + s + 1)
                                                + lgammal(a_a + s) + lgammal(b_a + nx - s) + lgammal(a_b + n - s) + lgammal(b_b + ny - n + s)) / logl(2.0) + base;
            ...
            if (config.test_type == "poisson_bayes") {
                ...
                m = const_cast<std::vector<std::pair<long double, size_t>>&&>(m_map.values_container());
                ...
                // if (config.test_type == "gpoisson") {
                //     m.erase(std::remove_if(m.begin(), m.end(), [&](const auto &a) { return !a.second; }), m.end());
                //     std::sort(m.rbegin(), m.rend());
                // }
            }
            ...
            if (m.size() > 1) {
                ...
                while (jt != m.end()) {
                    ...
                    if (jt->first - it->first > 1e-5) {
                        common::logger->error("min p-vals not sorted: {} vs. {}\t{} !>= {}",
                                              it - m.begin(), jt - m.begin(),
                                              it->first, jt->first);
                        throw std::runtime_error("p-val fail");
                    }
                    ++it;
        ...
        generate_clean_rows([&](uint64_t row_i, const auto &row, uint64_t bucket_idx) {
            ...
            if (config.test_type == "poisson_binom") {
                ...
                if (n > 0) {
                    ...
                    auto get_deviance = [&](long double x, long double mu, long double lambda) {
                    ...
                    auto get_total_deviance = [&](size_t s) {
                        ...
                        return get_deviance(s, mu_a, lambda_a) + get_deviance(n - s, mu_b, lambda_b);
                    };
                    ...
                    if (pval == 0.0) {
                        common::logger->trace("In: ({},{})\tOut: ({},{})\tNull: ({},{})\tn: {}\tin_sum: {}\tin_lprob: {} = {} + {} - {}",
                            mu_a,lambda_a,
                            mu_b,lambda_b,
                            mu_n,lambda_n,
                            n, in_sum, in_lprob,
                ...
                if (n != 0) {
                    ...
                    if (eff_size > 0) {
                        pval = boost::math::cdf(boost::math::complement(dist, in_sum - 1))
                                + boost::math::cdf(dist, floor(boost::math::quantile(boost::math::complement(dist, in_prob))));
                    } else if (eff_size < 0) {
                        pval = boost::math::cdf(dist, in_sum)
                                + boost::math::cdf(dist, ceil(boost::math::quantile(dist, in_prob)));
                    }
                ...
                if (n != 0) {
                    ...
                    long double base = (lgammal(n + 1) + lgammal(nx + ny - n + 1)
                                        - lgammal(a_n + n) - lgammal(b_n + nx + ny - n)) / logl(2.0) + bb_base;
                    uint64_t s = 0;
                    lprobs.emplace_back((-lgammal(s + 1) - lgammal(nx - s + 1)
                                        - lgammal(n - s + 1) - lgammal(ny - n + s + 1)
                                        + lgammal(a_a + s) + lgammal(b_a + nx - s) + lgammal(a_b + n - s) + lgammal(b_b + ny - n + s)) / logl(2.0) + base);

                    for (++s; s <= n; ++s) {
                        lprobs.emplace_back(lprobs.back()
                            - log2l(s + 1) + log2l(nx - (s - 1) + 1)
                            + log2l(n - (s - 1) + 1) - log2l(ny - n + s + 1)
                            + log2l(a_a + s) - log2l(b_a + nx - (s - 1)) - log2l(a_b + n - (s - 1)) + log2l(b_b + ny - n + s)
                        );
                    }
                    if (pval * k >= config.family_wise_error_rate)
                        return;
        ...
        if (config.test_type == "nbinom_exact" || config.test_type == "pig") {
            ...
            nb_pvals.erase(std::remove_if(nb_pvals.begin(), nb_pvals.end(),
                        ...
                        nb_pvals.end());
            common::logger->trace("Sorting {} / {} p-vals", nb_pvals.size(), kept.num_set_bits());
            ...
            for (size_t i = 0; i < nb_pvals.size(); ++i) {
                ...
                if (nb_pvals[i].first <= config.family_wise_error_rate * (i + 1) / nb_pvals.size() / harm) {
                    k = std::max(i + 1, k);
                    node_index node = AnnotatedDBG::anno_to_graph_index(nb_pvals[i].second);
                    if (!indicator_in[node] && !indicator_out[node]) {
                        common::logger->error("Node {} not marked, but p-value {} significant", node, nb_pvals[i].first);
            ...
            common::logger->trace("Found {} / {} significant p-values. Minimum p-value is {}",
                                k, nb_pvals.size(), nb_pvals[0].first);
            for (size_t i = k; i < nb_pvals.size(); ++i) {
                ...
                node_index node = AnnotatedDBG::anno_to_graph_index(row_i);
                if (!indicator_in[node] && !indicator_out[node]) {
                    common::logger->error("Node {} not marked, but original p-value {} significant", node, nb_pvals[i].first);
                    throw std::runtime_error("Index fail");
                }

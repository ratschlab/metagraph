...
encode_filter_suffix(const std::string &filter_suffix) {
    ...
    std::transform(filter_suffix.begin(), filter_suffix.end(),
                   ...
                   [&kmer_extractor](char c) { return kmer_extractor.encode(c); });
    return filter_suffix_encoded;
}

template <typename KmerCollector>
BitmapChunkConstructor<KmerCollector>::BitmapChunkConstructor(size_t k,
                                                              bool canonical_mode,
                                                              const std::string &filter_suffix,
                                                              size_t num_threads,
...
DBGBitmapConstructor::DBGBitmapConstructor(size_t k,
                                           ...
                                           double memory_preallocated,
                                           bool verbose)
    : constructor_(IBitmapChunkConstructor::initialize(k,
                                                       canonical_mode,
                                                       bits_per_count > 0,
                                                       filter_suffix,
                                                       num_threads,
...
template <typename KmerCollector>
sdsl::int_vector<> BitmapChunkConstructor<KmerCollector>::get_weights(uint8_t bits_per_count) {
...
DBGBitmap::Chunk *BitmapChunkConstructor<KmerCollector>::build_chunk() {
    ...
    std::unique_ptr<DBGBitmap::Chunk> chunk { new DBGBitmap::Chunk(
            ...
            (1llu << (get_k() * KMER::kBitsPerChar)) + 1, kmers.size()) };
    assert(chunk.get());

    return chunk.release();
}

DBGBitmap *DBGBitmapConstructor ::build_graph_from_chunks(
        uint64_t size,
        uint64_t num_kmers,
...
DBGBitmap *
DBGBitmapConstructor ::build_graph_from_chunks(const std::vector<std::string> &chunk_filenames,
                                               bool canonical_mode,
                                               bool verbose) {
    ...
    if (verbose)
        std::cout << "Cumulative size of chunks: " << cumulative_size - 1 << std::endl;
    ...
    return build_graph_from_chunks(
            ...
            [&]() {
                ...
                chunk_filename = utils::remove_suffix(chunk_filename,
                ...
                if (!chunk_in.good()) {
                    std::cerr << "ERROR: input file " << chunk_filename << " corrupted"
                              << std::endl;
                    exit(1);
                }
...
using KmerSet
        = kmer::KmerCollector<KMER, KmerExtractor2Bit, common::SortedSet<KMER, Vector<KMER>>>;
...
IBitmapChunkConstructor *IBitmapChunkConstructor ::initialize(size_t k,
                                                              bool canonical_mode,
                                                              bool count_kmers,
                                                              const std::string &filter_suffix,
                                                              size_t num_threads,

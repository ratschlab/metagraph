...
void sort_and_remove_duplicates(Array *array, size_t num_threads,
                                size_t offset) {
  ...
  // remove duplicates
  auto unique_end =
      std::unique(array->begin() + offset, array->end(),
                  utils::EqualFirst<typename Array::value_type>());
  array->erase(unique_end, array->end());
}

template <typename Array>
void shrink_kmers(Array *kmers, size_t num_threads, bool verbose,
                  size_t offset) {
  if (verbose) {
...
// k is node length
template <typename Array>
void recover_source_dummy_nodes(size_t k, Array *kmers, size_t num_threads,
                                bool verbose) {
  ...
  for (size_t c = 3; c < k + 1; ++c) {
    ...
    for (size_t i = succ_dummy_begin; i < dummy_begin; ++i) {
      ...
      push_back(*kmers, utils::get_first((*kmers)[i]))
          .to_prev(k + 1, BOSS::kSentinelCode);
    }
    sort_and_remove_duplicates(kmers, num_threads, dummy_begin);
...
encode_filter_suffix_boss(const std::string &filter_suffix) {
  ...
  std::transform(filter_suffix.begin(), filter_suffix.end(),
                 ...
                 [&kmer_extractor](char c) {
                   return c == BOSS::kSentinel ? BOSS::kSentinelCode
...
private:
  BOSSChunkConstructor(size_t k, bool canonical_mode = false,
                       const std::string &filter_suffix = "",
                       size_t num_threads = 1, double memory_preallocated = 0,
                       bool verbose = false)
      : kmer_storage_(k + 1, canonical_mode,
                      encode_filter_suffix_boss(filter_suffix), num_threads,
                      memory_preallocated, verbose) {
    if (filter_suffix == std::string(filter_suffix.size(), BOSS::kSentinel)) {
      kmer_storage_.insert_dummy(std::vector<KmerExtractorBOSS::TAlphabet>(
          k + 1, BOSS::kSentinelCode));
    }
  }
  ...
  BOSS::Chunk *build_chunk() {
    ...
    if constexpr (utils::is_pair<typename KmerStorage::Value>::value) {
      // kmer_collector stores (BOSS::k_ + 1)-mers
      result = new BOSS::Chunk(
          kmer_storage_.alphabet_size(), kmer_storage_.get_k() - 1,
          kmer_storage_.is_both_strands_mode(), kmers, kBitsPerCount);
    } else {
      // kmer_collector stores (BOSS::k_ + 1)-mers
      result = new BOSS::Chunk(kmer_storage_.alphabet_size(),
                               kmer_storage_.get_k() - 1,
                               kmer_storage_.is_both_strands_mode(), kmers);
    }
...
initialize_boss_chunk_constructor(size_t k, const Args &... args) {
  if ((k + 1) * KmerExtractorBOSS::bits_per_char <= 64) {
    return std::unique_ptr<IBOSSChunkConstructor>(
        new BOSSChunkConstructor<KmerContainer<KmerExtractorBOSS::Kmer64>>(
            k, args...));
  } else if ((k + 1) * KmerExtractorBOSS::bits_per_char <= 128) {
    return std::unique_ptr<IBOSSChunkConstructor>(
        new BOSSChunkConstructor<KmerContainer<KmerExtractorBOSS::Kmer128>>(
            k, args...));
  } else {
    return std::unique_ptr<IBOSSChunkConstructor>(
        new BOSSChunkConstructor<KmerContainer<KmerExtractorBOSS::Kmer256>>(
            k, args...));
  }
}

template <typename KMER>
using KmerCounterVector =
    KmerCounter<KMER, KmerExtractorBOSS, uint8_t,
                Vector<std::pair<KMER, uint8_t>>, utils::NoCleanup>;
template <typename KMER>
using KmerCounterVectorClean =
    KmerCounter<KMER, KmerExtractorBOSS, uint8_t,
                Vector<std::pair<KMER, uint8_t>>, utils::DummyKmersCleaner>;
template <typename KMER>
using KmerCollectorVector =
    KmerCollector<KMER, KmerExtractorBOSS, Vector<KMER>, utils::NoCleanup>;
template <typename KMER>
using KmerCollectorVectorClean =
    KmerCollector<KMER, KmerExtractorBOSS, Vector<KMER>,
...
using KmerCounterDeque =
    KmerCounter<KMER, KmerExtractorBOSS, uint8_t,
                ...
                utils::NoCleanup>;
template <typename KMER>
using KmerCounterDequeClean =
    KmerCounter<KMER, KmerExtractorBOSS, uint8_t,
                utils::DequeStorage<std::pair<KMER, uint8_t>>,
                utils::DummyKmersCleaner>;
template <typename KMER>
using KmerCollectorDeque =
    KmerCollector<KMER, KmerExtractorBOSS, utils::DequeStorage<KMER>,
                  utils::NoCleanup>;
template <typename KMER>
using KmerCollectorDequeClean =
    KmerCollector<KMER, KmerExtractorBOSS, utils::DequeStorage<KMER>,
                  utils::DummyKmersCleaner>;

std::unique_ptr<IBOSSChunkConstructor> IBOSSChunkConstructor ::initialize(
    size_t k, bool canonical_mode, bool count_kmers,
    const std::string &filter_suffix, size_t num_threads,
    double memory_preallocated, bool verbose) {
...
#define OTHER_ARGS                                                             \
  ...
  if (count_kmers) {
    ...
    if (filter_suffix.size()) {
      ...
      default:
        throw std::logic_error("Unsupported extractor container specified for "
                               "counter. Only VECTOR and DEQUE are supported");
      }
    } else {
      switch (kExtractorContainer) {
      case ExtractorContainer::VECTOR:
        return initialize_boss_chunk_constructor<KmerCounterVectorClean>(
            OTHER_ARGS);
      case ExtractorContainer::DEQUE:
        return initialize_boss_chunk_constructor<KmerCounterDequeClean>(
            OTHER_ARGS);
      default:
        throw std::logic_error("Unsupported extractor container specified for "
                               "counter. Only VECTOR and DEQUE are supported");
    ...
    if (filter_suffix.size()) {
      ...
      switch (kExtractorContainer) {
      case ExtractorContainer::VECTOR:
        return initialize_boss_chunk_constructor<KmerCollectorVector>(
            OTHER_ARGS);
      case ExtractorContainer::DEQUE:
        return initialize_boss_chunk_constructor<KmerCollectorDeque>(
            OTHER_ARGS);
      case ExtractorContainer::VECTOR_DISK:
        return initialize_boss_chunk_constructor<KmerCollectorVectorDisk>(
            OTHER_ARGS);
      default:
      ...
      switch (kExtractorContainer) {
      case ExtractorContainer::VECTOR:
        return initialize_boss_chunk_constructor<KmerCollectorVectorClean>(
            OTHER_ARGS);
      case ExtractorContainer::DEQUE:
        return initialize_boss_chunk_constructor<KmerCollectorDequeClean>(
            OTHER_ARGS);
      default:
        throw std::logic_error(

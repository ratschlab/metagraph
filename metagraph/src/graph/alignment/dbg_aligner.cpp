...
::align_batch(const std::vector<IDBGAligner::Query> &seq_batch,
    ...
    for (size_t i = 0; i < seq_batch.size(); ++i) {
        ...
        if (!config_.chain_alignments && graph_.get_extension_threadsafe<ColumnPathIndex>()) {
    ...
    if (try_connect && partial_alignments.size()) {
        ...
        for (size_t i = 1; i < partial_alignments.size(); ++i) {
            ...
            if (is_coord_jump(*first, next) || is_disconnected[i]) {
                ...
                if (const auto *labeled_aligner = dynamic_cast<const ILabeledAligner*>(this)) {
                    auto label_change_scores = labeled_aligner->get_label_change_scores(
                        merged.label_column_diffs.size() ? merged.label_column_diffs.back() : merged.label_columns,
                        next_fixed.label_columns
                    );

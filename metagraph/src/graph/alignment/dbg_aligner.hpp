...
class SeedAndExtendAligner : public IDBGAligner {
  ...
  protected:
    ...
    DBGQueryAlignment align_best_direction(const std::string_view query,
                                           bool orientation) const;
    ...
    DBGQueryAlignment align_both_directions(const std::string_view query,
                                            bool orientation) const;
...
::align_batch(const QueryGenerator &generate_query,
    ...
    generate_query([&](std::string_view header, std::string_view query, bool orientation) {
        ...
        if (get_graph().is_canonical_mode()) {
            ...
            callback(header, align_both_directions(query, orientation));
        } else if (get_config().forward_and_reverse_complement) {
            callback(header, align_best_direction(query, orientation));
        } else {
...
template <class Seeder, class Extender, class AlignmentCompare>
inline auto SeedAndExtendAligner<Seeder, Extender, AlignmentCompare>
::align_best_direction(const std::string_view query,
                       bool orientation) const -> DBGQueryAlignment {
    DBGQueryAlignment paths(query, orientation);
    ...
    align_aggregate(paths, [&](const auto &alignment_callback,
                               const auto &get_min_path_score) {
        align_core(forward,
                   build_alignment_core_generator(forward, false),
                   alignment_callback,
                   get_min_path_score);

        align_core(reverse,
                   build_alignment_core_generator(reverse, true),
                   alignment_callback,
                   get_min_path_score);
...
template <class Seeder, class Extender, class AlignmentCompare>
inline auto SeedAndExtendAligner<Seeder, Extender, AlignmentCompare>
::align_both_directions(const std::string_view query,
                        bool orientation) const -> DBGQueryAlignment {
    DBGQueryAlignment paths(query, orientation);

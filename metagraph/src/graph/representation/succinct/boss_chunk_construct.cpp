    ...
    for (TAlphabet F = 1; F < ALPHABET_LEN; ++F) {  // skip $$..$
        ...
        while (!it.empty()) {
            ...
            while (!sink_gen_it.empty()
                ...
                // sink k-mers and are redundant with the same dummy source
            ...
            if (!sink_gen_it.empty()) {
                ...
                if (KMER::compare_suffix(top, dummy_source, 1)) {
                    // The dummy source k-mer generated by a k-mer a is redundant with
                    // k-mer b iff the dummy sink generated by b is redundant with a. So
                    // #dummy_source being redundant implies that #top is also redundant

...
std::string convert_query_response_to_json(const std::string &ret_str) {
    // we are parsing back the string generated by the 'query' code, which is ugly.
...
std::string process_search_request(const std::string &received_message,
    ...
    if (!fasta.isNull()) {
        ...
        QueryExecutor qe(config, anno_graph, do_alignment ? aligner : nullptr, thread_pool);
...
int run_server(Config *config) {
    ...
    server.resource["^/search"]["POST"] = [&](shared_ptr<HttpServer::Response> response,
        ...
        process_request(response, request, [&](const std::string &content) {
            // TODO: smarter way to determine how many threads to use?
            ThreadPool query_thread_pool(num_threads);
            return process_search_request(content, *anno_graph, *config, aligner.get(),
                                          query_thread_pool);
        });
    };
    ...
    server.resource["^/align"]["POST"] = [&](shared_ptr<HttpServer::Response> response,
                                             shared_ptr<HttpServer::Request> request) {
        process_request(response, request, [&](const std::string &content) {
            return process_align_request(content, aligner.get());
        });
    };
    ...
    server.on_error = [](shared_ptr<HttpServer::Request> /*request*/,
        ...
        if (ec.value() != asio::stream_errc::eof
            ...
            logger->info("[Server] Got error " + ec.message() + " " + ec.category().name()
    ...
    logger->info(
            "[Server] Initializing a HTTP server with {} threads"
            ", listening on port {}",

...
void Config::print_usage(const std::string &prog_name, IdentityType identity) {
    ...
    switch (identity) {
        ...
        case ALIGN: {
            ...
            fprintf(stderr, "\t   --align-max-nodes-per-seq-char [FLOAT]\tmaximum number of nodes to consider per sequence character [2.0]\n");
            fprintf(stderr, "\t   --align-max-ram [FLOAT]\t\t\tmaximum amount of RAM used per alignment in MB [200.0]\n");
            fprintf(stderr, "\t   --align-xdrop [INT]\t\t\t\tthe maximum difference between the current score and the best alignment score [34]\n");

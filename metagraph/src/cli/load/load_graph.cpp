...
std::shared_ptr<DBGBitmap> load_critical_dbg_dd(const std::string &filename) {
    auto graph_type = parse_graph_type(filename);
    if (graph_type != Config::GraphType::BITMAP) {
        logger->error("load_dd for not BITMAP graph", filename);
        exit(1);
    }

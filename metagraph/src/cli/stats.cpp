...
void print_coord_to_accession_stats(const std::string &fname) {
    logger->info("Statistics for coord-to-accession mapping '{}'", fname);

    annot::CoordToAccession coord_to_accession;
    if (!coord_to_accession.load(fname)) {
        logger->error("Cannot load coord-to-accession mapping from file '{}'", fname);
        exit(1);
    }
    ...
    for (uint64_t col = 0; col < coord_to_accession.num_columns(); ++col) {
        total_num_kmers += coord_to_accession.num_kmers(col);
        total_num_sequences += coord_to_accession.num_sequences(col);
    }

    std::cout << "=============== COORD-TO-ACCESSION STATS ===============" << '\n';
    std::cout << "columns: " << coord_to_accession.num_columns() << '\n';
    ...
    if (get_verbose()) {
        ...
        for (uint64_t col = 0; col < coord_to_accession.num_columns(); ++col) {
            std::cout << "column " << col << ":\n";
            std::cout << "  sequences: " << coord_to_accession.num_sequences(col) << first_accessions(col) << '\n';
            std::cout << "  k-mers: " << coord_to_accession.num_kmers(col) << '\n';
            std::cout << "  k-mers per sequence: " << (coord_to_accession.num_sequences(col)
                ? fmt::format("{:.1f}", static_cast<double>(coord_to_accession.num_kmers(col))
                                            / coord_to_accession.num_sequences(col))
                : "NA") << '\n';
        }
    }
...
int print_stats(Config *config) {
    ...
    for (const auto &file : files) {
        ...
        if (utils::ends_with(file, annot::CoordToAccession::kExtension)) {
            print_coord_to_accession_stats(file);
            continue;
        }

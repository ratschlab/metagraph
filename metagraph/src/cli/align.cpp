...
DBGAlignerConfig initialize_aligner_config(const Config &config,
    ...
    DBGAlignerConfig c = {
        ...
        .max_dist_between_seeds = config.alignment_max_dist_between_seeds,
        .gap_shrinking_factor = config.alignment_gap_shrinking_factor,
        .min_path_score = config.alignment_min_path_score,
        .xdrop = config.alignment_xdrop,
        .label_change_scale_factor = config.alignment_label_change_scale_factor,
        .fixed_haplotype = -config.alignment_fixed_haplotype,
        ...
        .chains_per_char = config.alignment_chains_per_char,
        ...
        .ignore_discarded_seeds = config.alignment_ignore_discarded_seeds,
...
int align_to_graph(Config *config) {
    ...
    for (const auto &file : files) {
        ...
        while (it != end) {
            ...
            auto run_batch = [&,graph,batch=std::move(seq_batch)]() {
            ...
            if (get_num_threads() > 1) {
                thread_pool.enqueue(std::move(run_batch));
            } else {
                run_batch();
            }

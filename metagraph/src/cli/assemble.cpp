...
std::string sequence_to_gfa_path(std::string seq,
    ...
    graph->map_to_nodes(seq, [&](uint64_t node) { path_nodes.push_back(node); });
...
int assemble(Config *config) {
    ...
    if (config->to_gfa) {
        ...
        if (config->infbase != "") {
            logger->trace(
                    "Loading sequences for highlighting gfa paths from FASTA file '{}'",
                    config->infbase);
...
#pragma omp parallel for num_threads(get_num_threads()) schedule(dynamic) shared(gfa_file)
                for (size_t i = 0; i < seq_queries.size(); ++i) {
                    std::string path_string_gfa
                            = sequence_to_gfa_path(seq_queries[i], i, graph,
                                                   is_unitig_end_node, config);
                    std::lock_guard<std::mutex> lock(str_mutex);

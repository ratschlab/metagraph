...
class IntMatrix {
  public:
    typedef BinaryMatrix::Row Row;
    typedef BinaryMatrix::Column Column;
    ...
    virtual std::vector<RowValues> get_row_values(const std::vector<Row> &rows) const = 0;
...
class MultiIntMatrix : public IntMatrix {
  public:
    ...
    virtual std::vector<RowValues> get_row_values(const std::vector<Row> &rows) const;

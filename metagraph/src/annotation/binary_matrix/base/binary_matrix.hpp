...
class BinaryMatrix {
  public:
    ...
    virtual std::vector<sdsl::bit_vector> has_column(const std::vector<Row> &rows, const SetBitPositions &column) const;
    ...
    virtual Vector<size_t> extend_maximal(const std::vector<Row> &rows,

...
class KmerCollector {
  ...
  public:
    ...
    void add_sequence(std::string_view sequence, uint64_t count = 1) {
        ...
        if (sequence.size() >= k_)
            buffer_.push_and_pay(sequence.size(), sequence, count);
    }

    void add_sequence(std::string&& sequence, uint64_t count = 1) {
        // push read to the processing queue
        if (sequence.size() >= k_)
            buffer_.push_and_pay(sequence.size(), std::move(sequence), count);
    }
    ...
    /**
     * Sends sequences accumulated in #buffer_ for processing
     * on the thread pool. */
  ...
  private:
    ...
    BatchAccumulator<std::pair<std::string, uint64_t>> buffer_;

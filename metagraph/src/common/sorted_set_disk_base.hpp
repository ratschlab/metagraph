...
class SortedSetDiskBase {
  ...
  public:
    ...
    void start_merging() { // TODO: move to protected
        async_worker_.enqueue([this]() {
          ...
          std::function<void(const value_type &)> on_new_item = [this](const value_type
                  &v) { merge_queue_.push(v); };
  ...
  protected:
    virtual void sort_and_merge_duplicates(storage_type *vector, size_t num_threads) const = 0;

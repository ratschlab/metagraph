...
Config::Config(int argc, const char *argv[]) {
    ...
    const auto get_value = [&](int i) {
        ...
        if (i + 1 == argc) {
            std::cerr << "Error: no value provided for option " << argv[i] << std::endl;
            print_usage(argv[0], identity);
            exit(-1);
        }
        return argv[i + 1];
    ...
    if (!fname.size() && identity != STATS && identity != SERVER_QUERY
        && !(identity == BUILD && complete) && !(identity == CALL_VARIANTS)
        && !(identity == PARSE_TAXONOMY) && !(identity == CONCATENATE && !infbase.empty())) {
        std::string line;
    ...
    for (size_t i = 1; i < count_slice_quantiles.size(); ++i) {
        if (count_slice_quantiles[i - 1] >= count_slice_quantiles[i]) {
            std::cerr << "Error: bin count quantiles must be provided in strictly "
                         "increasing order"
                      << std::endl;
    ...
    if (count_slice_quantiles.front() < 0 || count_slice_quantiles.back() > 1) {
        std::cerr << "Error: bin count quantiles must be in range [0, 1]" << std::endl;
        print_usage_and_exit = true;
    }
    if (count_slice_quantiles.size() == 1) {
        std::cerr << "Error: provide at least two bin count borders" << std::endl;
        print_usage_and_exit = true;
    }
    ...
    if (identity != CONCATENATE && identity != STATS && identity != SERVER_QUERY
        && !(identity == BUILD && complete) && !(identity == CALL_VARIANTS)
        && !(identity == PARSE_TAXONOMY) && !fname.size())
        print_usage_and_exit = true;
    ...
    if (identity == ALIGN
        && (alignment_mm_transition < 0 || alignment_mm_transversion < 0
            || alignment_gap_opening_penalty < 0 || alignment_gap_extension_penalty < 0)) {
    ...
    if ((identity == ANNOTATE_COORDINATES || identity == ANNOTATE || identity == EXTEND)
        && infbase_annotators.size() > 1) {
        std::cerr << "Error: one annotator at most is allowed for extension." << std::endl;
        print_usage_and_exit = true;
    }

    if (identity == ANNOTATE && !filename_anno && !fasta_anno && !anno_labels.size()) {
        std::cerr << "Error: No annotation to add" << std::endl;
        print_usage_and_exit = true;
    }

    if ((identity == ANNOTATE || identity == ANNOTATE_COORDINATES) && outfbase.empty())
        outfbase = utils::remove_suffix(infbase, ".dbg", ".orhashdbg", ".hashstrdbg",
                                        ".bitmapdbg");
    ...
    if ((identity == TRANSFORM || identity == CLEAN || identity == ASSEMBLE
         || identity == RELAX_BRWT)
        && fname.size() != 1)
        print_usage_and_exit = true;

    if ((identity == TRANSFORM || identity == CONCATENATE || identity == EXTEND
         || identity == MERGE || identity == CLEAN || identity == TRANSFORM_ANNOTATION
         || identity == MERGE_ANNOTATIONS || identity == ASSEMBLE || identity == RELAX_BRWT)
        && outfbase.empty())
        print_usage_and_exit = true;
    ...
    if (alignment_max_seed_length < alignment_min_seed_length) {
        std::cerr
                << "Error: align-max-seed-length has to be at least align-min-seed-length"
                << std::endl;
        print_usage_and_exit = true;
    }
    ...
    if (initialize_bloom && bloom_bpk == 0.0 && bloom_fpp == 1.0) {
        std::cerr << "Error: at least one of 0.0 < bloom_fpp < 1.0 or 0.0 < bloom_bpk "
                     "must be true"
                  << std::endl;
        print_usage_and_exit = true;
    }
...
void Config::print_usage(const std::string &prog_name, IdentityType identity) {
    fprintf(stderr,
            "Metagraph: comprehensive metagenome graph representation -- Version "
            "0.1\n\n");

    const char annotation_list[]
            = "('column', 'row', 'bin_rel_wt_sdsl', 'bin_rel_wt', 'flat', 'rbfish', "
              "'brwt')";
    ...
    switch (identity) {
        case NO_IDENTITY: {
            ...
            fprintf(stderr,
                    "\textend\t\textend an existing graph with new sequences from\n");
            ...
            fprintf(stderr,
                    "\tconcatenate\tcombine the results of the external merge or\n");
            fprintf(stderr,
                    "\t\t\tconstruction and output the resulting graph structure\n\n");

            fprintf(stderr,
                    "\tcompare\t\tcheck whether two given graphs are identical\n\n");

            fprintf(stderr,
                    "\talign\t\talign sequences provided in fast[a|q] files to "
                    "graph\n\n");
            ...
            fprintf(stderr,
                    "\tcoordinate\tgiven a graph and a fast[a|q] file, annotate\n");
            fprintf(stderr,
                    "\t\t\tkmers with their respective coordinates in genomes\n\n");
            ...
            fprintf(stderr,
                    "\ttransform_anno\tchange representation of the graph "
                    "annotation\n\n");
            ...
            fprintf(stderr,
                    "\tserver_query\tannotate received sequences and send annotations "
                    "back\n\n");

            fprintf(stderr,
                    "\tcall_variants\tgenerate a masked annotated graph and call "
                    "variants\n");
            fprintf(stderr, "\t\t\trelative to unmasked graph\n\n");

            fprintf(stderr,
                    "\tparse_taxonomy\tgenerate NCBI Accession ID to Taxonomy ID "
                    "mapper\n\n");
        ...
        case BUILD: {
            fprintf(stderr,
                    "Usage: %s build [options] FILE1 [[FILE2] ...]\n"
            ...
            fprintf(stderr,
                    "\t   --min-count-q [INT] \tmin k-mer abundance quantile (min-count "
                    "is used by default) [0.0]\n");
            fprintf(stderr,
                    "\t   --max-count-q [INT] \tmax k-mer abundance quantile (max-count "
                    "is used by default) [1.0]\n");
            fprintf(stderr,
                    "\t   --reference [STR] \tbasename of reference sequence (for "
                    "parsing VCF files) []\n");
            fprintf(stderr,
                    "\t   --fwd-and-reverse \tadd both forward and reverse complement "
                    "sequences [off]\n");
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "\t   --graph [STR] \tgraph representation: succinct / bitmap / hash "
                    "/ hashpacked / hashstr [succinct]\n");
            fprintf(stderr,
                    "\t   --count-kmers \tcount k-mers and build weighted graph [off]\n");
            fprintf(stderr, "\t-k --kmer-length [INT] \tlength of the k-mer to use [3]\n");
            fprintf(stderr,
                    "\t-c --canonical \t\tindex only canonical k-mers (e.g. for read "
                    "sets) [off]\n");
            fprintf(stderr,
                    "\t   --complete \t\tconstruct a complete graph (only for Bitmap "
                    "graph) [off]\n");
            fprintf(stderr,
                    "\t   --mem-cap-gb [INT] \tpreallocated buffer size in Gb [0]\n");
            fprintf(stderr, "\t   --dynamic \t\tuse dynamic build method [off]\n");
            fprintf(stderr,
                    "\t-l --len-suffix [INT] \tk-mer suffix length for building graph "
                    "from chunks [0]\n");
            fprintf(stderr,
                    "\t   --suffix \t\tbuild graph chunk only for k-mers with the suffix "
                    "given [off]\n");
            fprintf(stderr, "\t-o --outfile-base [STR]\tbasename of output file []\n");
            fprintf(stderr,
                    "\t   --no-shrink \t\tdo not build mask for dummy k-mers (only for "
                    "Succinct graph) [off]\n");
            fprintf(stderr,
                    "\t-p --parallel [INT] \tuse multiple threads for computation [1]\n");
        } break;
        case CLEAN: {
            ...
            fprintf(stderr,
                    "\t   --max-count [INT] \t\tmax k-mer abundance, excluding [inf]\n");
            fprintf(stderr,
                    "\t   --prune-tips [INT] \t\tprune all dead ends shorter than this "
                    "value [1]\n");
            fprintf(stderr,
                    "\t   --prune-unitigs [INT] \tprune all unitigs with median k-mer "
                    "counts smaller\n"
                    "\t                         \t\tthan this value (0: auto) [1]\n");
            fprintf(stderr,
                    "\t   --fallback [INT] \t\tfallback threshold if the automatic one "
                    "cannot be determined [1]\n");
            fprintf(stderr,
                    "\t   --count-bins-q [FLOAT ...] \tbinning quantiles for "
                    "partitioning k-mers with\n"
                    "\t                              \t\tdifferent abundance levels ['0 "
                    "1']\n"
                    "\t                              \t\tExample: --count-bins-q '0 0.33 "
                    "0.66 1'\n");
            // fprintf(stderr, "\n");
            // fprintf(stderr, "\t-o --outfile-base [STR]\tbasename of output file []\n");
            fprintf(stderr,
            ...
            fprintf(stderr,
                    "\t   --to-fasta \t\t\tdump clean sequences to compressed FASTA file "
                    "[off]\n");
            // fprintf(stderr, "\t-p --parallel [INT] \tuse multiple threads for computation [1]\n");
        } break;
        case EXTEND: {
            fprintf(stderr,
                    "Usage: %s extend -i <GRAPH> -o <extended_graph_basename> [options] "
                    "FILE1 [[FILE2] ...]\n"
                    "\tEach input file is given in FASTA, FASTQ, VCF, or KMC format.\n"
                    "\tNote that VCF files must be in plain text or bgzip format.\n\n",
                    prog_name.c_str());
        ...
        case ALIGN: {
            fprintf(stderr,
                    "Usage: %s align -i <GRAPH> [options] FASTQ1 [[FASTQ2] ...]\n\n",
                    prog_name.c_str());

            fprintf(stderr,
                    "\t   --fwd-and-reverse \t\talign both forward and reverse "
                    "complement sequences [off]\n");
            fprintf(stderr,
                    "\t   --header-comment-delim [STR]\tdelimiter for joining fasta "
                    "header with comment [off]\n");
            fprintf(stderr,
                    "\t-p --parallel [INT] \tuse multiple threads for computation [1]\n");
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "\t   --map \t\t\tmap k-mers to graph exactly instead of "
                    "aligning.\n");
            fprintf(stderr,
                    "\t         \t\t\t\tTurned on if --count-kmers or --query-presence "
                    "are set [off]\n");
            fprintf(stderr,
                    "\t-k --kmer-length [INT]\t\tlength of mapped k-mers (at most "
                    "graph's k) [k]\n");
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "\t   --count-kmers \t\tfor each sequence, report the number of "
                    "k-mers discovered in graph [off]\n");
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "\t   --query-presence \t\ttest sequences for presence, report as 0 "
                    "or 1 [off]\n");
            fprintf(stderr,
                    "\t   --discovery-fraction [FLOAT] fraction of k-mers required to "
                    "count sequence [1.0]\n");
            fprintf(stderr,
                    "\t   --kmer-mapping-mode \t\tlevel of heuristics to use for mapping "
                    "k-mers (0, 1, or 2) [0]\n");
            fprintf(stderr,
                    "\t   --filter-present \t\treport only present input sequences as "
                    "FASTA [off]\n");
            fprintf(stderr, "\n");
            ...
            fprintf(stderr,
                    "\t   --align-alternative-alignments \t\tthe number of alternative "
                    "paths to report per seed [1]\n");
            fprintf(stderr,
                    "\t   --align-min-path-score [INT]\t\t\tthe minimum score that a "
                    "reported path can have [0]\n");
            fprintf(stderr,
                    "\t   --align-edit-distance \t\t\tuse unit costs for scoring matrix "
                    "[off]\n");
            fprintf(stderr,
                    "\t   --align-queue-size [INT]\t\t\tmaximum size of the priority "
                    "queue for alignment [50]\n");
            fprintf(stderr,
                    "\t   --align-vertical-bandwidth [INT]\t\tmaximum width of a window "
                    "to consider in alignment step [10]\n");
            fprintf(stderr, "\n");
            fprintf(stderr, "Advanced options for scoring:\n");
            fprintf(stderr,
                    "\t   --align-match-score [INT]\t\t\tpositive match score [2]\n");
            fprintf(stderr,
                    "\t   --align-mm-transition-penalty [INT]\t\tpositive transition "
                    "penalty (DNA only) [1]\n");
            fprintf(stderr,
                    "\t   --align-mm-transversion-penalty [INT]\tpositive transversion "
                    "penalty (DNA only) [2]\n");
            fprintf(stderr,
                    "\t   --align-gap-open-penalty [INT]\t\tpositive gap opening penalty "
                    "[3]\n");
            fprintf(stderr,
                    "\t   --align-gap-extension-penalty [INT]\t\tpositive gap extension "
                    "penalty [1]\n");
            fprintf(stderr,
                    "\t   --align-min-cell-score [INT]\t\t\tthe minimum value that a "
                    "cell in the alignment table can hold [0]\n");
            fprintf(stderr, "\n");
            fprintf(stderr, "Advanced options for seeding:\n");
            fprintf(stderr,
                    "\t   --align-seed-unimems \t\t\tuse maximal exact matches along "
                    "unitigs as seeds [off]\n");
            fprintf(stderr,
                    "\t   --align-min-seed-length [INT]\t\tthe minimum length of a seed "
                    "[graph k]\n");
            fprintf(stderr,
                    "\t   --align-max-seed-length [INT]\t\tthe maximum length of a seed "
                    "[graph k]\n");
            fprintf(stderr,
                    "\t   --align-max-num-seeds-per-locus [INT]\tthe maximum number of "
                    "allowed inexact seeds per locus [1]\n");
        } break;
        case COMPARE: {
        ...
        case MERGE: {
            fprintf(stderr,
                    "Usage: %s merge -o <graph_basename> [options] GRAPH1 GRAPH2 "
                    "[[GRAPH3] ...]\n\n",
                    prog_name.c_str());

            fprintf(stderr, "Available options for merge:\n");
            fprintf(stderr,
                    "\t-b --bins-per-thread [INT] \tnumber of bins each thread computes "
                    "on average [1]\n");
            fprintf(stderr,
                    "\t   --dynamic \t\t\tdynamic merge by adding traversed paths "
                    "[off]\n");
            fprintf(stderr,
                    "\t   --part-idx [INT] \t\tidx to use when doing external merge "
                    "[]\n");
            fprintf(stderr,
                    "\t   --parts-total [INT] \t\ttotal number of parts in external "
                    "merge[]\n");
            fprintf(stderr,
        ...
        case CONCATENATE: {
            fprintf(stderr,
                    "Usage: %s concatenate -o <graph_basename> [options] [[CHUNK] "
                    "...]\n\n",
                    prog_name.c_str());

            fprintf(stderr, "Available options for merge:\n");
            fprintf(stderr,
                    "\t   --graph [STR] \tgraph representation: succinct / bitmap "
                    "[succinct]\n");
            fprintf(stderr,
                    "\t-i --infile-base [STR] \tload graph chunks from files "
                    "'<infile-base>.<suffix>.<type>.chunk' []\n");
            fprintf(stderr,
                    "\t-l --len-suffix [INT] \titerate all possible suffixes of the "
                    "length given [0]\n");
            fprintf(stderr,
                    "\t-c --canonical \t\tcanonical graph mode (e.g. for read sets) "
                    "[off]\n");
            fprintf(stderr,
                    "\t   --no-postprocessing \tdo not erase redundant dummy edges after "
                    "concatenation [off]\n");
            fprintf(stderr,
                    "\t-p --parallel [INT] \tuse multiple threads for computation [1]\n");
        } break;
        case TRANSFORM: {
            ...
            // fprintf(stderr, "\t-o --outfile-base [STR] basename of output file []\n");
            fprintf(stderr,
                    "\t   --clear-dummy \terase all redundant dummy edges [off]\n");
            fprintf(stderr,
                    "\t   --prune-tips [INT] \tprune all dead ends of this length and "
                    "shorter [0]\n");
            fprintf(stderr,
                    "\t   --state [STR] \tchange state of succinct graph: fast / faster "
                    "/ dynamic / small [fast]\n");
            fprintf(stderr, "\t   --to-adj-list \twrite adjacency list to file [off]\n");
            fprintf(stderr,
                    "\t   --to-fasta \t\textract sequences from graph and dump to "
                    "compressed FASTA file [off]\n");
            fprintf(stderr,
                    "\t   --initialize-bloom \tconstruct a Bloom filter for faster "
                    "detection of non-existing k-mers [off]\n");
            fprintf(stderr,
                    "\t   --unitigs \t\textract all unitigs from graph and dump to "
                    "compressed FASTA file [off]\n");
            fprintf(stderr,
            ...
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "Advanced options for --initialize-bloom. bloom-fpp, when < 1, "
                    "overrides bloom-bpk.\n");
            fprintf(stderr,
                    "\t   --bloom-fpp [FLOAT] \t\t\t\texpected false positive rate "
                    "[1.0]\n");
            fprintf(stderr,
                    "\t   --bloom-bpk [FLOAT] \t\t\t\tnumber of bits per kmer [4.0]\n");
            fprintf(stderr,
                    "\t   --bloom-max-num-hash-functions [INT] \tmaximum number of hash "
                    "functions [10]\n");
        } break;
        case ASSEMBLE: {
            fprintf(stderr,
                    "Usage: %s assemble -o <outfile-base> [options] GRAPH\n"
                    "\tAssemble contigs from de Bruijn graph and dump to compressed "
                    "FASTA file.\n\n",
                    prog_name.c_str());
            ...
            fprintf(stderr,
                    "\t   --label-mask-out [STR] \t\tlabel to exclude from masked "
                    "graph\n");
            fprintf(stderr,
                    "\t   --label-mask-in-fraction [FLOAT] \tminimum fraction of mask-in "
                    "labels among the set of masked labels [1.0]\n");
            fprintf(stderr,
                    "\t   --label-mask-out-fraction [FLOAT] \tmaximum fraction of "
                    "mask-out labels among the set of masked labels [0.0]\n");
            fprintf(stderr,
                    "\t   --label-other-fraction [FLOAT] \tmaximum fraction of other "
                    "labels allowed [1.0]\n");
            fprintf(stderr,
                    "\t   --filter-by-kmer \t\t\tmask out graph k-mers individually "
                    "[off]\n");
        } break;
        case STATS: {
            ...
            fprintf(stderr,
                    "\t   --print-internal \tprint internal graph representation to "
                    "screen [off]\n");
            fprintf(stderr,
                    "\t   --count-dummy \tshow number of dummy source and sink edges "
                    "[off]\n");
            fprintf(stderr, "\t-a --annotator [STR] \tannotation []\n");
            fprintf(stderr,
                    "\t   --print-col-names \tprint names of the columns in annotation "
                    "to screen [off]\n");
            fprintf(stderr,
                    "\t-p --parallel [INT] \tuse multiple threads for computation [1]\n");
        } break;
        case ANNOTATE: {
            fprintf(stderr,
                    "Usage: %s annotate -i <GRAPH> [options] FILE1 [[FILE2] ...]\n"
            ...
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "\t   --anno-type [STR] \ttarget annotation representation: column / "
                    "row [column]\n");
            fprintf(stderr, "\t-a --annotator [STR] \tannotator to update []\n");
            fprintf(stderr,
                    "\t   --sparse \t\tuse the row-major sparse matrix to annotate graph "
                    "[off]\n");
            fprintf(stderr,
                    "\t-o --outfile-base [STR] basename of output file [<GRAPH>]\n");
            fprintf(stderr,
                    "\t   --separately \tannotate each file independently and dump to "
                    "the same directory [off]\n");
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "\t   --anno-filename \t\tinclude filenames as annotation labels "
                    "[off]\n");
            fprintf(stderr,
                    "\t   --anno-header \t\textract annotation labels from headers of "
                    "sequences in files [off]\n");
            fprintf(stderr,
                    "\t   --header-comment-delim [STR]\tdelimiter for joining fasta "
                    "header with comment [off]\n");
            fprintf(stderr,
                    "\t   --header-delimiter [STR]\tdelimiter for splitting annotation "
                    "header into multiple labels [off]\n");
            fprintf(stderr,
                    "\t   --anno-label [STR]\t\tadd label to annotation for all "
                    "sequences from the files passed []\n");
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "\t-p --parallel [INT] \tuse multiple threads for computation [1]\n");
        ...
        case ANNOTATE_COORDINATES: {
            fprintf(stderr,
                    "Usage: %s coordinate -i <GRAPH> [options] FASTA1 [[FASTA2] ...]\n\n",
                    prog_name.c_str());
            ...
            fprintf(stderr,
                    "\t-o --outfile-base [STR] \tbasename of output file [<GRAPH>]\n");
            fprintf(stderr,
                    "\t   --coord-binsize [INT]\tstepsize for k-mer coordinates in input "
                    "sequences from the fasta files [1000]\n");
        ...
        case MERGE_ANNOTATIONS: {
            fprintf(stderr,
                    "Usage: %s merge_anno -o <annotator_basename> [options] ANNOT1 "
                    "[[ANNOT2] ...]\n\n",
                    prog_name.c_str());
            ...
            fprintf(stderr,
                    ...
                    "[column]\n");
            fprintf(stderr, "\t\t");
            fprintf(stderr, annotation_list);
            fprintf(stderr, "\n");
        ...
        case TRANSFORM_ANNOTATION: {
            fprintf(stderr,
                    "Usage: %s transform_anno -o <annotator_basename> [options] "
                    "ANNOTATOR\n\n",
                    prog_name.c_str());

            // fprintf(stderr, "\t-o --outfile-base [STR] basename of output file []\n");
            fprintf(stderr,
                    "\t   --rename-cols [STR] \tfile with rules for renaming annotation "
                    "labels []\n");
            ...
            fprintf(stderr, "\t                       \t          ... ...........'\n");
            fprintf(stderr,
                    "\t   --anno-type [STR] \ttarget annotation format [column]\n");
            fprintf(stderr, "\t\t");
            fprintf(stderr, annotation_list);
            fprintf(stderr, "\n");
            fprintf(stderr, "\t   --arity \t\tarity in the brwt tree [2]\n");
            fprintf(stderr,
                    "\t   --greedy \t\tuse greedy column partitioning in brwt "
                    "construction [off]\n");
            fprintf(stderr,
                    "\t   --fast \t\ttransform annotation in memory without streaming "
                    "[off]\n");
            fprintf(stderr,
                    "\t   --dump-raw-anno \tdump the columns of the annotator as "
                    "separate binary files [off]\n");
            fprintf(stderr,
                    "\t   --dump-text-anno \tdump the columns of the annotator as "
                    "separate text files [off]\n");
            fprintf(stderr,
                    "\t-p --parallel [INT] \tuse multiple threads for computation [1]\n");
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "\t   --parallel-nodes [INT] \tnumber of nodes processed in parallel "
                    "in brwt tree [n_threads]\n");
        } break;
        case RELAX_BRWT: {
            fprintf(stderr,
                    "Usage: %s relax_brwt -o <annotator_basename> [options] "
                    "ANNOTATOR\n\n",
                    prog_name.c_str());

            fprintf(stderr, "\t-o --outfile-base [STR] basename of output file []\n");
            fprintf(stderr,
                    "\t   --relax-arity [INT] \trelax brwt tree to optimize arity "
                    "limited to this number [10]\n");
            fprintf(stderr,
                    "\t-p --parallel [INT] \tuse multiple threads for computation [1]\n");
        } break;
        case QUERY: {
            fprintf(stderr,
                    "Usage: %s query -i <GRAPH> -a <ANNOTATION> [options] FILE1 [[FILE2] "
                    "...]\n"
            ...
            fprintf(stderr,
                    "\t   --fwd-and-reverse \tquery both forward and reverse complement "
                    "sequences [off]\n");
            fprintf(stderr,
                    "\t   --align \t\talign sequences instead of mapping k-mers.\n");
            fprintf(stderr,
                    "\t   --sparse \t\tuse row-major sparse matrix for row annotation "
                    "[off]\n");
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "\t   --count-labels \t\tcount labels for k-mers from querying "
                    "sequences [off]\n");
            fprintf(stderr,
                    "\t   --num-top-labels \t\tmaximum number of frequent labels to "
                    "print [off]\n");
            fprintf(stderr,
                    "\t   --discovery-fraction [FLOAT] fraction of labeled k-mers "
                    "required for annotation [1.0]\n");
            fprintf(stderr,
                    "\t   --labels-delimiter [STR]\tdelimiter for annotation labels "
                    "[\":\"]\n");
            fprintf(stderr,
                    "\t   --suppress-unlabeled \tdo not show results for sequences "
                    "missing in graph [off]\n");
            // fprintf(stderr, "\t-d --distance [INT] \tmax allowed alignment distance [0]\n");
            fprintf(stderr, "\n");
            fprintf(stderr,
                    "\t-p --parallel [INT] \tuse multiple threads for computation [1]\n");
            fprintf(stderr,
                    "\t   --cache-size [INT] \tnumber of uncompressed rows to store in "
                    "the cache [0]\n");
        ...
        case SERVER_QUERY: {
            fprintf(stderr,
                    "Usage: %s server_query -i <GRAPH> -a <ANNOTATION> [options]\n\n",
                    prog_name.c_str());
            ...
            // fprintf(stderr, "\t-d --distance [INT] \tmax allowed alignment distance [0]\n");
            fprintf(stderr,
                    "\t-p --parallel [INT] \tmaximum number of parallel connections "
                    "[1]\n");
            fprintf(stderr,
                    "\t   --cache-size [INT] \tnumber of uncompressed rows to store in "
                    "the cache [0]\n");
        ...
        case CALL_VARIANTS: {
            fprintf(stderr,
                    "Usage: %s call_variants -i <GRAPH> -a <annotation> [options]\n",
                    prog_name.c_str());
            ...
            fprintf(stderr,
                    ...
                    "[]\n");
            fprintf(stderr,
                    "\t   --label-mask-out [STR] \t\tlabel to exclude from masked graph "
                    "[]\n");
            fprintf(stderr,
                    "\t   --label-mask-in-fraction [FLOAT] \tminimum fraction of mask-in "
                    "labels among the set of masked labels [1.0]\n");
            ...
            fprintf(stderr,
                    "\t   --call-breakpoints \t\tcall labels from breakpoints [off]\n");
            fprintf(stderr,
                    "\t   --label-filter [STR] \tdiscard variants with this label []\n");
            fprintf(stderr,
                    "\t   --taxonomy-map [STR] \tfilename of taxonomy map file []\n");
            fprintf(stderr,
                    "\t   --cache-size [INT] \t\tnumber of uncompressed rows to store in "
                    "the cache [0]\n");
        } break;
        case PARSE_TAXONOMY: {
            ...
            fprintf(stderr, "\t-o --outfile-base [STR] basename of output file []\n");
            fprintf(stderr,
                    "\t   --accession [STR] \tfilename of the accession2taxid.gz file "
                    "[]\n");

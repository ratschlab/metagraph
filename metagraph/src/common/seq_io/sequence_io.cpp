...
FastaWriter::FastaWriter(const std::string &filename, const std::string &header,
                         bool write_counts)
    : header_(header), write_counts_(write_counts) {
...
FastaWriter::~FastaWriter() { gzclose(gz_out_); }

void FastaWriter::write(const std::string &sequence) {
  if (!write_fasta(gz_out_,
                   write_counts_ ? header_ + std::to_string(++count_) : header_,
                   sequence)) {
    std::cerr
        << "ERROR: FastaWriter::write failed. Can't dump sequence to fasta"
        << std::endl;
    exit(1);
  }
}
...
void read_fasta_file_critical(gzFile input_p,
  ...
  if (read_stream == NULL) {
    std::cerr << "ERROR: failed to initialize kseq file descriptor"
              << std::endl;
    exit(1);
  }
...
void read_vcf_file_critical(const std::string &filename,
                            const std::string &ref_filename, size_t k,
                            std::function<void(std::string &&)> callback,
                            bool with_reverse) {
  VCFParser vcf(ref_filename, filename, k);

  if (with_reverse) {
    vcf.call_sequences([&](auto &&sequence) {
...
void read_vcf_file_with_annotations_critical(
    const std::string &filename, const std::string &ref_filename, size_t k,
    std::function<void(std::string &&, const std::vector<std::string> &)>
        callback,
    bool with_reverse) {
  // TODO: make this a configurable option
  // default list of tokens to extract as annotations
  // TODO: extract these guys directly from vcf parsed
  const std::vector<std::string> annots = {"AC_AFR", "AC_EAS", "AC_AMR",
                                           "AC_ASJ", "AC_FIN", "AC_NFE",
                                           "AC_SAS", "AC_OTH"};

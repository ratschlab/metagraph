...
class sd_vector_disk
{
    ...
    public:
        ...
        sd_vector_disk(sd_vector_disk_builder& builder)
        {
            ...
            std::ofstream out(builder.filename, std::ios::ate | std::ios::binary);
            int64_t wb = (m_size*m_wl+7)/8;
            size_t m_low_endpos = builder.file_offset + sizeof(m_size) + sizeof(m_wl) + 9 + wb + (wb%8 ? 8-wb%8 : 0);
            if (out.tellp() != m_low_endpos)
...
class select_0_support_sd_disk
{
    public:
        typedef bit_vector::size_type size_type;
    ...
    public:
        ...
        explicit select_0_support_sd_disk(const bit_vector_type* v=nullptr)
        {
            set_vector(v);
            if (nullptr != m_v) {
        ...
        select_0_support_sd_disk& operator=(const select_0_support_sd_disk& ss)
        {
            if (this != &ss) {
        ...
        void swap(select_0_support_sd_disk& ss)
        {

...
uint64_t atomic_exchange(sdsl::int_vector<> &vector,
                         uint64_t i,
                         uint64_t val,
    ...
    if (width == 64)
        return __atomic_exchange_n(vector.data() + i, val, mo);

    if (width == 32)
        return __atomic_exchange_n((uint32_t*)vector.data() + i, val, mo);

    if (width == 16)
        return __atomic_exchange_n((uint16_t*)vector.data() + i, val, mo);

    if (width == 8)
        return __atomic_exchange_n((uint8_t*)vector.data() + i, val, mo);

...
/** Clear the high bits ov #value starting at position #index. */
uint64_t bzhi(uint64_t value, uint32_t index);

/**
 * Elias-Fano encoder that streams the encoded result into a file.
 * Inspired by Folly's https://github.com/facebook/folly/blob/master/folly/experimental/EliasFanoCoding.h
 */
template <typename T>
class EliasFanoEncoder {
  public:
    ...
    size_t finish() {
        ...
        if (num_lower_bits_ != 0) {
            size_t cur_pos_bytes = (cur_pos_bits_ + 7) / 8;
  ...
  private:
    ...
    /**
     * Returns number of lower bits for a sorted array of size #size and maximum value
     * max_value.
     */
    uint8_t get_num_lower_bits(T max_value, size_t size) {
  ...
  private:
     ...
     * To save memory, only the 16 bytes are kept in memory. As soon as a chunk of 8
    ...
    /**
     * Number of elements added so far to the encoder.
     */
    size_t size_ = 0;
...
class EliasFanoDecoder {
  public:
    ...
    T next_lower() {
        ...
        return bzhi(ptrv >> (adjusted_pos % 8), num_lower_bits_);
    }

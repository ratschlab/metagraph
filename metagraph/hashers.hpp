...
class ExactFilter {
  ...
  private:
    ...
    uint64_t hash_(Object *a, Object *b) {
        ...
        uint64_t first_hash;
        std::copy(reinterpret_cast<uint64_t*>(&hash),
                  reinterpret_cast<uint64_t*>(&hash) + 1, &first_hash);
        return first_hash;
    }
...
class BloomFilter {
  ...
  public:
    ...
    */
    ...
    bool operator==(const BloomFilter<Size> &a) const {
        if (n_bits != a.n_bits) {
...
class HashAnnotation {
  public:
    ...
    */
    ...
    template <typename T, typename S>
    std::vector<size_t> insert(T *a, T *b,
                               S begin, S end,
                               bool change = false,
                               bool query = false) {
    ...
    template <typename T>
    std::vector<size_t> find(T *a, T *b, size_t ind) {
    ...
    */
    ...
    template <typename T>
    void annotate(const T &a,
                  std::vector<uint64_t> &annot,
                  bool op_or_and = false) const {
        for (auto it = color_bits.begin(); it != color_bits.end(); ++it) {
            if (op_or_and) {
                //bitwise AND
                if (!(it->find(a))) {
                    annot[(it - color_bits.begin()) >> 6]
                        &= (-1llu ^ (1llu << ((it - color_bits.begin()) % 64)));
                }
            } else {
                //bitwise OR
                if (it->find(a)) {
                    annot[(it - color_bits.begin()) >> 6]
                        |= (1llu << ((it - color_bits.begin()) % 64));
                }
            }
        }
    }

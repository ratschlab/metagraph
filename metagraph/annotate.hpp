...
namespace annotate {
    ...
    void annotate_seq(DBG_succ *G, kstring_t &seq, kstring_t &label,
                      uint64_t start = 0, uint64_t end = 0, pthread_mutex_t *anno_mutex = NULL);

    void annotate_kmer(DBG_succ *G, std::string &kmer, uint32_t &label,
                       uint64_t &previous, pthread_mutex_t *anno_mutex, bool ignore = false);

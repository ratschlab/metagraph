...
then
    echo "/usr/bin/time -v $exe transform -v --to-fasta --contigs -p 1 -o ${FILE}.k$K.contigs ${FILE}.k$K" | bsub -M $mem -n 1 -We 12:00 -R "rusage[mem=${mem}]" -J kmc_msub2 -o ${FILE}.kmc2contS2.k${K}.cluster.log 
fi

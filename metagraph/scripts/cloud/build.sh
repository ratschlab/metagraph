...
exit_code=0
set +e
if execute metagraph build -v -p 4 -k 31 --canonical --state small --count-kmers --no-shrink -o "${output_dir}/${sra_id}" --mem-cap-gb ${mem_cap_gb} --disk-swap ${tmp_dir} --disk-cap-gb 200 --count-width 16 ${input_dir}/${sra_id}.kmc.kmc_pre; then
  exit_code=0
else
  exit_code=1
fi

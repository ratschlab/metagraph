...
#TOOD: set back fallback to "${fallback}"
execute metagraph clean -v -p 2 --min-count 1 --num-singletons "${num_singletons}"  --prune-unitigs 0 --fallback 5 --prune-tips 62 --to-fasta -o "${output_file}" "${input_file}"

    ...
    std::string decode(path_id path) const {
        ...
        while (true) {
            if (this->node_is_split(node)) {
                encoded_base
                        = this->routing_table.get(node, relative_position); // maybe different
                base = this->routing_table.traversed_base(node, relative_position);
                relative_position
                        = this->routing_table.new_relative_position(node, relative_position);
            ...
            if (this->node_is_join(node)) {
                if constexpr (use_char) {
                    ...
                    relative_position
                            += this->incoming_table.branch_offset(node, join_symbol);
                } else {
...
#pragma omp parallel for reduction(append : reads)
    ...
    path_id get_global_path_id(node_index node, int64_t relative_position) const {
        ...
        if (this->node_is_split(prev_node)) {
            relative_position = this->routing_table.select(
                    prev_node, relative_position + 1,
                    this->node_get_last_char(node)); // +1 as relative_position is 0-based
        }
        return get_global_path_id(prev_node, relative_position);
    }

    bool is_valid_path_id(path_id path_id) const {
        return this->node_is_join(path_id.first)
                && path_id.second
                < this->incoming_table.branch_size(path_id.first, origin_node_symbol);
    }

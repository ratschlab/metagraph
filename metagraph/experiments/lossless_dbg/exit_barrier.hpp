  ...
  public:
...
#endif
    {
        if (get_num_threads() > max_threads) {
            throw std::runtime_error("Implementation doesn't work with more than "s
                                     + to_string(max_threads) + " threads"s);
        }
    ...
    void update_others_before_sleep(exit_barrier_t &exit_barrier, int tid) {
        ...
        for (int i = 0; i < max_threads; i++) {
            ...
            //            }
            // vectorized version
            other_relative_position += other_edge == my_edge
                    and other_relative_position >= my_relative_position and i != tid;
        }
    }
    ...
    int64_t update_myself_after_wakeup(exit_barrier_t &exit_barrier, int tid) {
        ...
        for (int i = 0; i < max_threads; i++) {
            ...
            //            }
            offset_change -= other_edge == my_edge
                    and other_relative_position < my_relative_position and i != tid;
        }
    ...
    void print_content(uint64_t node,
                       ...
                       = -1) const { // print_content and exit have to have the same signature
...
ostream &operator<<(ostream &os, const waiting_thread_info_t &wt) {
    os << "[tid=" << wt.thread_id << ",rp=" << wt.relative_position << ",node=" << wt.node
       << ",tedg=" << wt.traversed_edge << "]";
    return os;
}
...
class ReferenceExitBarrier {
  public:
    ReferenceExitBarrier(BitVector *is_element,
                         RankSupport *rank_is_element,
                         int chunk_size = DefaultChunks)
        : waiting_threads(is_element, rank_is_element, chunk_size),
          waiting_queue_locks(is_element, rank_is_element, chunk_size) {
    ...
    int64_t exit(uint64_t node, int tid) {
        ...
        auto result = update_myself_after_wakeup(tid, myself.node, myself.traversed_edge,
                                                 waiting_queue, myself.relative_position);
    ...
    void enter(uint64_t node, char traversed_edge, int64_t relative_position, int tid) {
        ...
        auto waiting_queue_lock = waiting_queue_locks.ptr_to(node);
        omp_set_lock(waiting_queue_lock);
        waiting_threads[node].push_back(
                { (int16_t)tid, relative_position, node, traversed_edge });
        omp_unset_lock(waiting_queue_lock);
    }
    ...
    int64_t update_myself_after_wakeup(int tid,
        ...
        for (auto &other : waiting_queue) {
            if (was_me) {
                // future
                if ((int64_t)other.node == previous_node
                    and other.traversed_edge == traversed_edge and other.thread_id < 0
                    and // job has already finished

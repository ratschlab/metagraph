...
class standardized_wavelet_tree {
  public:
    ...
    standardized_wavelet_tree(uint8_t logsigma, const Vector &vector)
        : wt_(logsigma, vector) {}

    uint64_t rank(uint64_t i, uint64_t c) const {
        return i == 0 ? 0 : wt_.rank(c, i - 1);
    }
...
class SolidDynamicRoutingTable {
  public:
    ...
    int64_t offset(node_index node) const {
        ...
        return routing_table.select(node + 1, delimiter_encoded) + 1;
    } // node+1 as select is one based
    ...
    int64_t select_unchecked(node_index node, int64_t occurrence, int64_t encoded_symbol) const {
        ...
        auto result = routing_table.select(occurrences_of_symbol_before_block + occurrence,
                                           encoded_symbol)
                - routing_table_block;
        assert(result >= 0);
        return result;
    ...
    int64_t rank(node_index node, int64_t position, char symbol, ll hint_block_offset) const {
        ...
        auto occurrences_of_base_before_block
                = routing_table.rank(hint_block_offset, encode(symbol));
    ...
    ll insert(ll block, int64_t position, char symbol) {
        ...
        assert((position <= size(block) || [&]() {
            PRINT_VAR(block, position, symbol, size(block), encoded, symbol);
            return false;
        }()));

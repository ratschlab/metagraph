...
class NoisySampler : public SamplerConvenient {
  public:
    NoisySampler(string reference, std::mt19937 generator, double probability_of_error = 0)
        : reference(std::move(reference)),
          generator(generator),
          probability_of_error(probability_of_error),
          err({ 1 - probability_of_error, probability_of_error / 3,
                probability_of_error / 3, probability_of_error / 3 }) {};
    ...
    int64_t reference_size() override { return reference.size(); }
...
class SubSampler : public NoisySampler {
  public:
    SubSampler(const string &reference,
               int64_t subsample_size,
               std::mt19937 generator,
               double probability_of_error = 0)
        : NoisySampler(reference.substr(std::uniform_int_distribution<>(
                                                0,
                                                reference.length()
                                                        - min(subsample_size,
                                                              (int64_t)reference.length()))(
                                                generator),
...
class DeterministicSampler : public SamplerConvenient {
  public:
    DeterministicSampler(vector<string> samples, int64_t reference_size)
        : _reference_size(reference_size), samples(std::move(samples)) {};
    string sample(int64_t sample_length) override {
    ...
    int64_t reference_size() override { return _reference_size; }

...
void compress_store_reads(ValueArg<std::string> &graphArg,
                          const ValueArg<std::string> &statisticsArg,
                          ValueArg<std::string> &compressedArg,
                          const string &statistics_filename,
                          const vector<string> &reads,
                          shared_ptr<DBGSuccinct> &graph,
                          int chunks,
...
template <class DatabaseT>
void compress_reads(ValueArg<std::string> &graphArg,
                    const ValueArg<std::string> &statisticsArg,
                    const string &statistics_filename,
                    const vector<string> &reads,
                    shared_ptr<DBGSuccinct> &graph,
...
int main_compressor(int argc, char *argv[]) {
    ...
    TCLAP::ValueArg<std::string> graphArg("g", "graph",
                                          ...
                                          true, "", "string", cmd);
    std::vector<std::string> rerouting { "yes", "no" };
    ValuesConstraint<std::string> rerouting_constraint(rerouting);
    TCLAP::ValueArg<std::string> pathReroutingArg(
            "r", "path-rerouting", "Enable rerouting to improve space efficiency.", false,
            "yes", &rerouting_constraint, cmd);
    ...
    TCLAP::ValueArg<std::string> statisticsArg(
            ...
            false, "statistics.json", "filename", cmd);
    TCLAP::ValueArg<int> verbosityArg("x", "statistics-verbosity",
                                      "Level of detail of the statistics", false, 0u,
                                      "int64_t", cmd);
    TCLAP::ValueArg<std::string> compressedArg(
            "o", "output", "Folder where to store the compressed files.", false, "",
            "filename", cmd);
    TCLAP::ValueArg<int> reducedCoverageArg("v", "reduced-coverage",
                                            "Enable / disable reduced coverage technique",
                                            false, 1, "filename", cmd);
    TCLAP::ValueArg<int> numThreadsArg(
            "p", "threads", "Number of threads to use for parallel computation.", false,
            1, "int", cmd);
    TCLAP::ValueArg<int> chunksArg(
            "u", "chunks",
            "Number of chunks for routing and incoming table (less chunks decreases "
            "memory consumption but increases compression time) -1 = inf disables "
            "chunking.",
            false, DefaultChunks, "int", cmd);
    ...
    ValueArg<std::string> compressor_type("c", "compressor-type",
                                          "Which compressor to choose", false, "wavelet",
    ...
    if (compressor == "wavelet_debug") {
        compress_store_reads<DynamicPathDatabaseCore<
                ...
                graphArg, statisticsArg, compressedArg, statistics_filename, reads, graph,
                chunks, stat_verbosity);
    }
    if (compressor == "wavelet") {
        if (use_transformations) {
            if (chunks > 0) {
                compress_store_reads<PathDatabaseWavelet<>>(graphArg, statisticsArg,
                                                            compressedArg,
                                                            statistics_filename, reads,
                                                            graph, chunks, stat_verbosity);
            } else {
            ...
            if (chunks > 0) {
                if (reducedCoverageArg.getValue()) {
                    compress_store_reads<PathDatabaseWaveletWithtoutTransformation<true>>(
                            graphArg, statisticsArg, compressedArg, statistics_filename,
                            reads, graph, chunks, stat_verbosity);
                } else {
                    compress_store_reads<PathDatabaseWaveletWithtoutTransformation<false>>(
                            graphArg, statisticsArg, compressedArg, statistics_filename,
                            reads, graph, chunks, stat_verbosity);
                }
            } else {
                if (not reducedCoverageArg.getValue()) {
                    throw std::runtime_error("Not implemented");
                }
                compress_store_reads<PathDatabaseWaveletWithtoutTransformation<
                        true, ReferenceDynamicRoutingTable<>, ReferenceDynamicIncomingTable<>>>(
                        graphArg, statisticsArg, compressedArg, statistics_filename,
                        reads, graph, chunks, stat_verbosity);
            }
        }

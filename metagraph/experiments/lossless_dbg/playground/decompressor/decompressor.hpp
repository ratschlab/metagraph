...
vector<string> decompress(fs::path input_folder,
                          const fs::path &output_filename,
                          fs::path statistics_filename) {
...
int main_decompressor(int argc, char *argv[]) {
    ...
    TCLAP::ValueArg<std::string> outputArg("o", "output",
                                           "FASTA file to write the output to", true, "",
                                           "string", cmd);
    ...
    TCLAP::ValueArg<std::string> pathReroutingArg(
            "r", "path-rerouting", "Was path rerouting used for compression?", false,
            "yes", "<yes|no>", cmd);
    cmd.parse(argc, argv);
    ...
    if (pathReroutingArg.getValue() == "yes") {
        ...
        decompress<PathDatabaseWaveletWithtoutTransformation<>>(input_folder, output_filename,
                                                                statistics_filename);
    }

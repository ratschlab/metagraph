...
int main_sampler(int argc, char *argv[]) {
    ...
    TCLAP::ValueArg<std::string> referenceArg(
            "r", "reference",
            "Path to the reference from which the sampler will sample (FASTA file)", true,
            "", "string", cmd);
    TCLAP::ValueArg<double> coverageArg("c", "coverage", "Coverage of reads", false, 1.0,
                                        "double", cmd);
    TCLAP::ValueArg<double> error_probabilityArg("p", "error-probability",
                                                 "Probability of a substitution error",
                                                 false, 0.0, "double", cmd);
    TCLAP::ValueArg<int> read_lengthArg("l", "read-length", "Length of individual reads",
                                        false, 100, "int64_t", cmd);
    TCLAP::ValueArg<int> subsampleArg(
            "z", "subsample-size",
            "Subsample the reference to 'subsample-size' base pairs.", false, -1,
            "int64_t", cmd);
    TCLAP::ValueArg<int> seedArg("s", "seed", "Seed for a random generator", false, 12345,
                                 "int64_t", cmd);
    TCLAP::ValueArg<std::string> outputArg("o", "output",
                                           "Filename where to store sampled reads.",
                                           false, "output.fasta", "string", cmd);
    cmd.parse(argc, argv);
    ...
    NoisySampler sampler = subsampleArg.isSet()
            ? SubSampler(reference, subsample_size, generator, error_probability)

...
class TransformationsEnabler : public RoutingTable {
  public:
    ...
    TransformationsEnabler(shared_ptr<const DBGSuccinct> graph, Args... args)
        : RoutingTable(graph, args...) {
#ifndef DISABLE_TRANSFORMATIONS
        transformations = GraphPreprocessor(*graph).find_weak_splits();
#endif
    }
    ...
    //        transformations = GraphPreprocessor(graph).find_weak_splits();
    //                                            }
    explicit TransformationsEnabler(transformations_t transformations)

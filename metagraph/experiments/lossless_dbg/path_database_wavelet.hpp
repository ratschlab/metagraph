...
class PathDatabaseWaveletCore : public DynamicPathDatabaseCore<DynamicRoutingTableT,DynamicIncomingTableT> {
public:
    ...
    PathDatabaseWaveletCore(std::shared_ptr<const DBGSuccinct> graph,int chunks=DefaultChunks) : DynamicPathDatabaseCore<DRT,DIT>(graph, chunks), routing_table(graph)
                                                                              {}

    PathDatabaseWaveletCore(const vector<string> &filenames,
                        size_t kmer_length = 21 /* default */) : DynamicPathDatabaseCore<DRT,DIT>(filenames, kmer_length), routing_table(this->graph_)
                                                         {}
...
#pragma omp parallel
        ...
        statistics["routing_table_to_vector_time"] = transforming_routing_table_t.timer.elapsed();
        ...
        if constexpr (std::is_base_of<TransformationsEnabler<RoutingTableCore<>>,RoutingTableT>::value) {
            routing_table.transformations = DynamicPathDatabaseCore< DRT, DIT>::routing_table.transformations;
        }
        statistics["transformation_routing_table_time"] = transforming_routing_table_t.finished();;
...
#pragma omp parallel
        ...
        for(uint64_t node=1;node<=this->graph.num_nodes();node++) {
            ...
            if (DynamicPathDatabaseCore<DRT,DIT>::node_is_join(node)) {
                auto new_reads = DynamicPathDatabaseCore<DRT,DIT>::incoming_table.branch_size(node, '$');
                ...
                for(auto& base : "ACGTN") {
                    auto branch_size = DynamicPathDatabaseCore<DRT,DIT>::incoming_table.branch_size(node,base);
                    if (branch_size) {
                ...
                this->graph.call_incoming_kmers(node,[&,this](node_index /* prev_node */,char c) { //mine
                    auto branch_size = c == '$' ? 0 : DynamicPathDatabaseCore<DRT,DIT>::incoming_table.branch_size(node, c); // if dummy kmers (fix proper test)
                    incoming_table_builder_local.push_back(branch_size);
                    current_table_size++;
                    delimiter_vector_local.push_back(false);
                });
        ...
        statistics["transformation_incoming_table_time"] = incoming_table_transformations_t.finished();

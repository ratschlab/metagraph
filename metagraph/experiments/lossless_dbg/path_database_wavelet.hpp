...
template <class RoutingTableT = DefaultRoutingTable,
          class IncomingTableT = DefaultIncomingTable,
          bool reduced_coverage = true,
          class DynamicRoutingTableT = typename DynamicVersion<RoutingTableT>::Type,
          class DynamicIncomingTableT = typename DynamicVersion<IncomingTableT>::Type>
class PathDatabaseWaveletCore
    : public DynamicPathDatabaseCore<DynamicRoutingTableT, DynamicIncomingTableT> {
  public:
    ...
    PathDatabaseWaveletCore(std::shared_ptr<const DBGSuccinct> graph,
                            int chunks = DefaultChunks)
        : DynamicPathDatabaseCore<DRT, DIT>(graph, chunks), routing_table(graph) {}

    PathDatabaseWaveletCore(const vector<string> &filenames,
                            size_t kmer_length = 21 /* default */)
        : DynamicPathDatabaseCore<DRT, DIT>(filenames, kmer_length),
          routing_table(this->graph_) {}


    std::vector<path_id> encode(const std::vector<std::string> &sequences) {
        auto encoded = DynamicPathDatabaseCore<DRT, DIT>::encode(sequences);
    ...
    void construct_routing_table() {
        VerboseTimer transforming_routing_table_t("transforming routing_table");
...
#pragma omp for
            for (uint64_t node = 1; node <= this->graph.num_nodes(); node++) {
                ...
                if (DynamicPathDatabaseCore<DRT, DIT>::node_is_split(node)) {
                    auto &dynamic_table = DynamicPathDatabaseCore<DRT, DIT>::routing_table;
...
#pragma omp barrier
                if (t == omp_get_thread_num()) {
                    routing_table_array.insert(routing_table_array.end(),
                                               all(routing_table_array_local));
                }
            }
        };
        routing_table_array.push_back('#'); // to also always end a block with #
        statistics["routing_table_to_vector_time"]
                = transforming_routing_table_t.timer.elapsed();
        statistics["routing_table_to_vector_ram"] = get_used_memory();
        routing_table.initialize_content(routing_table_array);
        if constexpr (std::is_base_of<TransformationsEnabler<RoutingTableCore<>>,
                                      RoutingTableT>::value) {
            routing_table.transformations
                    = DynamicPathDatabaseCore<DRT, DIT>::routing_table.transformations;
        }
        statistics["transformation_routing_table_time"]
                = transforming_routing_table_t.finished();
        ;
        statistics["transformation_routing_table_ram"] = get_used_memory();
    }

    void construct_incoming_table() {
        VerboseTimer incoming_table_transformations_t("transforming incoming_table");
...
#pragma omp for
            for (uint64_t node = 1; node <= this->graph.num_nodes(); node++) {
                ...
                if (DynamicPathDatabaseCore<DRT, DIT>::node_is_join(node)) {
                    auto new_reads
                            = DynamicPathDatabaseCore<DRT, DIT>::incoming_table.branch_size(
                                    node, '$');
                    ...
                    for (auto &base : "ACGTN") {
                        auto branch_size = DynamicPathDatabaseCore<DRT, DIT>::incoming_table
                                                   .branch_size(node, base);
                        if (branch_size) {
...
#else
                    this->graph.call_incoming_kmers(
                            node, [&, this](node_index /* prev_node */, char c) { // mine
                                auto branch_size = c == '$'
                                        ? 0
                                        : DynamicPathDatabaseCore<DRT, DIT>::incoming_table
                                                  .branch_size(node,
                                                               c); // if dummy kmers (fix proper test)
                                incoming_table_builder_local.push_back(branch_size);
                                current_table_size++;
                                delimiter_vector_local.push_back(false);
                            });
                    ...
                    assert((current_table_size > 0 || [&]() {
                        DynamicPathDatabaseCore<DRT, DIT>::incoming_table.print_content(node);
                        PRINT_VAR(DynamicPathDatabaseCore<DRT, DIT>::node_is_join(node));
                        return false;
                    }()));
...
#pragma omp barrier
                if (t == omp_get_thread_num()) {
                    incoming_table_builder.insert(incoming_table_builder.end(),
                                                  all(incoming_table_builder_local));
                    delimiter_vector.insert(delimiter_vector.end(),
                                            all(delimiter_vector_local));
                }
            }
        };
        ...
        incoming_table = decltype(incoming_table)(this->graph_,
                                                  joins_type(temporary_representation),
                                                  int_representation);
        statistics["transformation_incoming_table_time"]
                = incoming_table_transformations_t.finished();
        statistics["transformation_incoming_table_ram"] = get_used_memory();
    }


    // is join or start of the read
    bool node_is_join(node_index node) const { return incoming_table.is_join(node); }
    ...
    void serialize(const fs::path &folder) {
        VerboseTimer serializing_path_encoder_t("serializing the path encoder");
        ...
        alt_assert(this->graph_->get_node_sequence(1)
                   != std::string(this->graph_->get_k(), '$'));
#else
        alt_assert(this->graph_->get_node_sequence(1)
                   == std::string(this->graph_->get_k(), '$'));
#endif
        ...
        serializing_path_encoder_t.finished();
    }
    template <typename Database
              = QueryEnabler<DecodeEnabler<PathDatabaseWaveletCore<RoutingTableT, IncomingTableT>>>>
    static Database deserialize(const fs::path &folder) {
        ...
        auto graph = std::shared_ptr<DBGSuccinct> { new DBGSuccinct(21) };
...
#pragma omp parallel for num_threads(get_num_threads())
    ...
    json get_statistics(uint64_t verbosity = ~0u) const {
        ...
        json result = DynamicPathDatabaseCore<DRT, DIT>::get_statistics(verbosity);
        ...
        if (verbosity > 0) {
            ...
            json addition = { { "added_joins", added_joins },
...
template <class RoutingTableT = DefaultRoutingTable,
          ...
          class DynamicRoutingTableT = typename DynamicVersion<RoutingTableT>::Type,
          class DynamicIncomingTableT = typename DynamicVersion<IncomingTableT>::Type>
using PathDatabaseWavelet = QueryEnabler<DecodeEnabler<
        PathDatabaseWaveletCore<RoutingTableT, IncomingTableT, reduced_coverage, DynamicRoutingTableT, DynamicIncomingTableT>>>;

template <bool reduced_coverage = true,
          class DynamicRoutingTableT = typename DynamicVersion<RoutingTableCore<>>::Type,
          class DynamicIncomingTableT = typename DynamicVersion<DefaultIncomingTable>::Type>
using PathDatabaseWaveletWithtoutTransformation = QueryEnabler<DecodeEnabler<
        PathDatabaseWaveletCore<RoutingTableCore<>, DefaultIncomingTable, reduced_coverage, DynamicRoutingTableT, DynamicIncomingTableT>>>;

...
typedef ::testing::Types<sdsl::uint128_t, uint64_t, uint32_t> ValueTypes;
...
TEST(EliasFano, ReadEmpty) {
    utils::TempFile out;
    common::EliasFanoEncoder<uint64_t> encoder(0, std::numeric_limits<uint64_t>::max(),
                                               out.ofstream());
    encoder.finish();

    common::EliasFanoDecoder<uint64_t> decoder(out.ifstream());
    EXPECT_FALSE(decoder.next().has_value());
}

TEST(EliasFano, WriteOne) {
    utils::TempFile out;
    common::EliasFanoEncoder<uint64_t> encoder(1, 1234, out.ofstream());
    encoder.add(1234);
    size_t file_size = encoder.finish();
...
TEST(EliasFano, ReadOne) {
    utils::TempFile file;
    common::EliasFanoEncoder<uint64_t> encoder(1, 1234, file.ofstream());
    encoder.add(1234);
    encoder.finish();

    common::EliasFanoDecoder<uint64_t> decoder(file.ifstream());
    std::optional<int64_t> decoded = decoder.next();
    EXPECT_TRUE(decoded.has_value());
    EXPECT_EQ(static_cast<uint64_t>(1234), decoded.value());
    EXPECT_FALSE(decoder.next().has_value());
}

TEST(EliasFano, WriteTwo) {
    utils::TempFile out;
    common::EliasFanoEncoder<uint64_t> encoder(2, 4321, out.ofstream());
    encoder.add(1234);
    encoder.add(4321);
    size_t file_size = encoder.finish();
...
TEST(EliasFano, ReadTwo) {
    utils::TempFile file;
    common::EliasFanoEncoder<uint64_t> encoder(2, 4321, file.ofstream());
    encoder.add(1234);
    encoder.add(4321);
    encoder.finish();
    ...
    EXPECT_EQ(static_cast<uint64_t>(4321), decoded.value());
    EXPECT_FALSE(decoder.next().has_value());
}

TEST(EliasFano, ReadWriteIncrementOne) {
    Vector<uint64_t> values(100);
    std::iota(values.begin(), values.end(), 0);
    utils::TempFile file;
    common::EliasFanoEncoder<uint64_t> encoder(values, file.ofstream());
    size_t file_size = encoder.finish();
    ...
    for (uint32_t i = 0; i < 100; ++i) {
        std::optional<int64_t> decoded = decoder.next();
        EXPECT_TRUE(decoded.has_value());
        EXPECT_EQ(static_cast<uint64_t>(i), decoded.value());
    }
    EXPECT_FALSE(decoder.next().has_value());
}

TEST(EliasFano, ReadWriteIncrementTwo) {
    Vector<uint64_t> values(100);
    uint32_t i = 0;
    std::for_each(values.begin(), values.end(), [&i](uint64_t &v) { v = 2 * i++; });
    utils::TempFile file;
    common::EliasFanoEncoder<uint64_t> encoder(values, file.ofstream());
    size_t file_size = encoder.finish();
    EXPECT_EQ(file_size, std::filesystem::file_size(file.name()));
    ...
    for (uint32_t i = 0; i < 100; ++i) {
        std::optional<int64_t> decoded = decoder.next();
        EXPECT_TRUE(decoded.has_value());
        EXPECT_EQ(static_cast<uint64_t>(2 * i), decoded.value());
    }
    EXPECT_FALSE(decoder.next().has_value());
}

TEST(EliasFano, VariousSizes) {
    for (uint32_t size = 100; size < 116; ++size) {
        Vector<uint64_t> values(size);
        std::iota(values.begin(), values.end(), 0);
        utils::TempFile file;
        common::EliasFanoEncoder<uint64_t> encoder(values, file.ofstream());
        size_t file_size = encoder.finish();
 ...
 */
TEST(EliasFano, ReadWriteExactly64LowBits) {
    Vector<uint64_t> values = { 1,   5,   7,   12,  16,  17,  25,  31,  32,  37,  40,
...
TEST(EliasFano, ReadWriteExactly128LowBits) {
...
TEST(EliasFano, ReadWriteRandom) {
    for (uint32_t size = 1000; size < 1016; ++size) {
        ...
        for (uint32_t trial = 0; trial < 16; ++trial) {
            ...
            uint32_t i = 0;
            std::for_each(values.begin(), values.end(), [&](uint64_t &v) {
                i += dist10(rng);
                v = i;
            });
            utils::TempFile file;
